...
use panopticon_graph_algos::{AdjacencyList, AdjacencyMatrixGraphTrait,
                             GraphTrait, MutableGraphTrait,
...
impl Program {
    ...
    pub fn find_function_by_uuid<'a>(
        &'a self,
        a: &Uuid,
    ) -> Option<&'a Function> {
        self.call_graph
    ...
    /// Returns the function with UUID `a`.
    pub fn find_function_by_uuid_mut<'a>(
        &'a mut self,
        a: &Uuid,
    ...
    pub fn insert(&mut self, new_ct: CallTarget) -> Vec<Uuid> {
        ...
        let mut ret = Vec::new();
        let calls = if let Some(&CallTarget::Concrete(ref fun)) =
            self.call_graph.vertex_label(new_vx) {
            fun.collect_calls()
        } else {
            vec![]
        };
        ...
        for a in calls {
            ...
            for w in self.call_graph.vertices() {
                match self.call_graph.vertex_label(w) {
                    Some(&CallTarget::Concrete(Function {
                                                  cflow_graph: ref cg,
                                                  entry_point: Some(ent),
                                                  ..
                                              })) => {
    ...
    pub fn find_call_target_by_uuid<'a>(
        &'a self,
        uu: &Uuid,
    ) -> Option<CallGraphRef> {
...
mod tests {
    use super::*;
    use {BasicBlock, ControlFlowTarget, Function, Lvalue, Mnemonic, Operation,
         Rvalue, Statement};
    ...
    fn find_by_entry() {
        ...
        func.entry_point =
            Some(func.cflow_graph.add_vertex(ControlFlowTarget::Resolved(bb0)));

        prog.call_graph
            .add_vertex(
                CallTarget::Concrete(
                    Function::new("test".to_string(), "ram".to_string()),
                )
            );
    ...
    fn insert_replaces_todo() {
        ...
        let tvx =
            prog.call_graph.add_vertex(
                CallTarget::Todo(Rvalue::new_u64(12), None, uu),
            );
        let vx0 = prog.call_graph
            .add_vertex(
                CallTarget::Concrete(
                    Function::new("test".to_string(), "ram".to_string()),
                )
            );
        let vx1 = prog.call_graph
            .add_vertex(
                CallTarget::Concrete(
                    Function::new("test2".to_string(), "ram".to_string()),
                )
            );
        ...
        let mut func = Function::with_uuid(
            "test3".to_string(),
            uu.clone(),
            "ram".to_string(),
        );
        ...
        if let Some(&CallTarget::Concrete(ref f)) =
            prog.call_graph.vertex_label(tvx) {
    ...
    fn insert_ignores_new_todo() {
        ...
        let tvx =
            prog.call_graph.add_vertex(
                CallTarget::Todo(Rvalue::new_u64(12), None, uu1),
            );
