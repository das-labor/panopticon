...
impl MnemonicFormatToken {
    fn parse_bank<'a>(mut i: Chars<'a>) -> Result<(String, Chars<'a>)> {
        ...
        if i.next() == Some(':') {
            ...
            Err(
                "Mnemonic format string parse error. Bank name is invalid."
                    .into()
            )
        }
    }
        ...
        loop {
            match j.next() {
                ...
                Some('{') => {
                    match j.next() {
                        Some('{') => {
                            ret.push(MnemonicFormatToken::Literal('{'))
                        }
                        Some('u') => {
                            ret.push(
                                MnemonicFormatToken::Variable {
                                    has_sign: false,
                                }
                            );
                            j.next();
...
mod tests {
    ...
    fn parse_format_string() {
        ...
        assert!(
            MnemonicFormatToken::parse("{69:+}".to_string().chars()).is_err()
        );
        assert!(
            MnemonicFormatToken::parse("{-69:+}".to_string().chars()).is_err()
        );
        ...
        assert!(
            MnemonicFormatToken::parse("{69::ddd".to_string().chars()).is_err()
        );
        assert_eq!(
