...
impl<A: Architecture> State<A> {
    ...
    pub fn jump(&mut self, v: Rvalue, g: Guard) -> Result<()> {
        if !(self.mnemonics.is_empty() ||
             self.mnemonics.last().unwrap().area.len() > 0) {
            return Err(
    ...
    fn eq(&self, other: &Rule<A>) -> bool {
        match (self, other) {
            (&Rule::Terminal {
                 mask: ref ma,
                 pattern: ref pa,
                 capture_group: ref ca,
             },
             &Rule::Terminal {
                 mask: ref mb,
                 pattern: ref pb,
                 capture_group: ref cb,
             }) => ma == mb && pa == pb && ca == cb,
            (&Rule::Sub(ref a), &Rule::Sub(ref b)) => {
                a.as_ref() as *const Disassembler<A> as usize ==
                b.as_ref() as *const Disassembler<A> as usize
            }
            _ => false,
        }
    }
...
impl<A: Architecture> Disassembler<A> {
    ...
    {
        let mut matches =
            self.find(i.clone(), &State::<A>::new(offset, cfg.clone()));
        let l = matches.len();
                            ...
                            Some(&Rule::Terminal {
                                ...
                                if let Some(tok) = Self::read_token(&mut i) {
                                    if mask.clone() & tok.clone() == *pattern {
                                        ...
                                        // capture group
                                        for &(ref name, ref mask) in
                                            capture.iter() {
                                            let mut res = if let Some(p) =
                                                st.groups.iter().position(
                                                    |x| {
                                                        x.0 == *name
                                                    }
                                                ) {
...
impl<'a, A: Architecture> Into<Rule<A>> for &'a str {
    fn into(self) -> Rule<A> {
        ...
        for c in self.chars() {
            match c {
                '@' => {
                    if read_pat {
                        panic!(
                            "Pattern syntax error: read '@' w/o name in '{}'",
                            self
                        );
                    } else {
                        read_pat = true;
                ...
                '.' => {
                    ...
                    if read_pat && cur_group != "" {
                        *groups.get_mut(&cur_group).unwrap() =
                            groups.get(&cur_group).unwrap().clone() |
                            (A::Token::one() << ((bit - 1) as usize));
                    }
        ...
        if bit != 0 {
            panic!(
                "Pattern syntax error: invalid pattern length in '{}'",
                self
            );
        }
...
mod tests {
    ...
    fn fixture()
        -> (Arc<Disassembler<TestArchShort>>,
            Arc<Disassembler<TestArchShort>>,
            Arc<Disassembler<TestArchShort>>,
            OpaqueLayer)
    {
    ...
    fn single_decoder() {
        ...
        if let &(0, Rvalue::Constant { value: 1, size: 64 }, ref g) =
            &res.jumps[0] {
    ...
    fn sub_decoder() {
        let (_, _, main, def) = fixture();
        let maybe_res =
            main.next_match(&mut def.iter().cut(&(1..def.len())), 1, ());
        ...
        if let &(1, Rvalue::Constant { value: 3, size: 64 }, ref g) =
            &res.jumps[0] {
            assert_eq!(g, &Guard::always());
        } else {
            assert!(false);
    ...
    fn semantic_false() {
        ...
        let maybe_res =
            sub2.next_match(&mut def.iter().cut(&(7..def.len())), 7, ());
    ...
    fn default_pattern() {
        let (_, _, main, def) = fixture();
        let maybe_res =
            main.next_match(&mut def.iter().cut(&(7..def.len())), 7, ());
        ...
        if let &(7, Rvalue::Constant { value: 8, size: 64 }, ref g) =
            &res.jumps[0] {
            assert_eq!(g, &Guard::always());
        } else {
            assert!(false);
    ...
    fn slice() {
        ...
        if let &(1, Rvalue::Constant { value: 2, size: 64 }, ref g) =
            &res.jumps[0] {
            assert_eq!(g, &Guard::always());
        } else {
            assert!(false);
    ...
    fn capture_group() {
        let (_, _, main, def) = fixture();
        let maybe_res =
            main.next_match(&mut def.iter().cut(&(4..def.len())), 4, ());
        ...
        if let &(4, Rvalue::Constant { value: 5, size: 64 }, ref g) =
            &res.jumps[0] {
            assert_eq!(g, &Guard::always());
        } else {
            assert!(false);
    ...
    fn empty_capture_group() {
        ...
        assert!(
            res.groups == vec![("a".to_string(), 3), ("c".to_string(), 7)] ||
            res.groups == vec![("c".to_string(), 7), ("a".to_string(), 3)]
        );
        assert_eq!(res.mnemonics.len(), 1);
