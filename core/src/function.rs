...
use {Architecture, BasicBlock, Guard, Mnemonic, Operation, Region, Rvalue,
     Statement};

use panopticon_graph_algos::{AdjacencyList, EdgeListGraphTrait, GraphTrait,
...
fn index_cflow_graph(g: ControlFlowGraph, maybe_entry: Option<u64>) -> (BTreeMap<u64,Vec<MnemonicOrError>>,HashMap<u64,Vec<(Rvalue,Guard)>>,HashMap<u64,Vec<(Rvalue,Guard)>>){
        ...
        if let Some(entry) = maybe_entry {
            by_destination.insert(
                entry,
                vec![(Rvalue::Undefined, Guard::always())],
            );
        }
            ...
            match g.vertex_label(v) {
                Some(&ControlFlowTarget::Resolved(ref bb)) => {
                    ...
                    for mne in &bb.mnemonics {
                        mnemonics
                            ...
                            .or_insert(Vec::new())
                            .push(MnemonicOrError::Mnemonic(mne.clone()));
                        ...
                        if let Some(prev) = prev_mne {
                            by_source
                                ...
                                .or_insert(Vec::new())
                                .push(
                                    (Rvalue::new_u64(mne.area.start),
                                     Guard::always())
                                );
                            by_destination
                                .entry(mne.area.start)
                                .or_insert(Vec::new())
                                .push((Rvalue::new_u64(prev), Guard::always()));
                        }
                ...
                Some(&ControlFlowTarget::Failed(ref pos, ref msg)) => {
                    mnemonics
                        .entry(*pos)
                        .or_insert(Vec::new())
                        .push(MnemonicOrError::Error(*pos, msg.clone()));
                }
        ...
        for e in g.edges() {
            ...
            match (src, tgt) {
                // Resolved -> Resolved
                (Some(&ControlFlowTarget::Resolved(ref src_bb)),
                 Some(&ControlFlowTarget::Resolved(ref tgt_bb))) => {
                    let last = src_bb.mnemonics.last().map_or(
                        src_bb.area.start,
                        |mne| mne.area.start,
                    );
                    by_source
                        .entry(last)
                        .or_insert(Vec::new())
                        .push((Rvalue::new_u64(tgt_bb.area.start), gu.clone()));
                    by_destination
                        .entry(tgt_bb.area.start)
                        .or_insert(Vec::new())
                        .push((Rvalue::new_u64(last), gu));
                }
                ...
                (Some(&ControlFlowTarget::Resolved(ref src_bb)),
                 Some(&ControlFlowTarget::Unresolved(Rvalue::Constant {
                    ...
                    by_source
                        .entry(last)
                        .or_insert(Vec::new())
                        .push((Rvalue::new_u64(*c), gu.clone()));
                    by_destination
                        .entry(*c)
                        .or_insert(Vec::new())
                        .push((Rvalue::new_u64(last), gu));
                ...
                (Some(&ControlFlowTarget::Unresolved(Rvalue::Constant {
                 ...
                 Some(&ControlFlowTarget::Resolved(ref tgt_bb))) => {
                    ...
                    by_destination
                        .entry(tgt_bb.area.start)
                        .or_insert(Vec::new())
                        .push((Rvalue::new_u64(*c), gu));
                }
                // Resolved -> Unresolved
                (Some(&ControlFlowTarget::Resolved(ref src_bb)),
                 Some(&ControlFlowTarget::Unresolved(ref r))) => {
                    by_source
                        .entry(src_bb.area.start)
                        .or_insert(Vec::new())
                        .push((r.clone(), gu));
                }
                // Unresolved -> Resolved
                (Some(&ControlFlowTarget::Unresolved(ref t)),
                 Some(&ControlFlowTarget::Resolved(ref tgt_bb))) => {
                    by_destination
                        .entry(tgt_bb.area.start)
                        .or_insert(Vec::new())
                ...
                // Failed -> Resolved
                (Some(&ControlFlowTarget::Failed(ref src_pos, _)),
                 Some(&ControlFlowTarget::Resolved(ref tgt_bb))) => {
                    by_source
                        .entry(*src_pos)
                        .or_insert(Vec::new())
                    ...
                    by_destination
                        .entry(tgt_bb.area.start)
                        .or_insert(Vec::new())
                        .push((Rvalue::new_u64(*src_pos), gu));
                }
                // Resolved -> Failed
                (Some(&ControlFlowTarget::Resolved(ref src_bb)),
                 Some(&ControlFlowTarget::Failed(ref tgt_pos, _))) => {
                    let last = src_bb.mnemonics.last().map_or(
                        src_bb.area.start,
                        |mne| mne.area.start,
                    );
                    by_source
                        .entry(last)
                        .or_insert(Vec::new())
                        .push((Rvalue::new_u64(*tgt_pos), gu.clone()));
                    by_destination
                        .entry(*tgt_pos)
                        .or_insert(Vec::new())
                        .push((Rvalue::new_u64(last), gu));
    ...
    fn assemble_cflow_graph(
        ...
        for (_, mnes) in mnemonics.iter_mut() {
            if !bblock.is_empty() && !mnes.is_empty() {
                if let Some(&MnemonicOrError::Mnemonic(ref mne)) =
                    mnes.first() {
                    let last_mne = &bblock.last().unwrap().clone();

                    // if next mnemonics aren't adjacent
                    let mut new_bb = bblock.last().unwrap().area.end !=
                                     mne.area.start;
                    ...
                    new_bb |= by_source
                        ...
                        .iter()
                        .any(
                            |&(ref opt_dest, _)| if let &Rvalue::Constant {
                                       value, ..
                                   } = opt_dest {
                    ...
                    new_bb |= by_destination
                        ...
                        .iter()
                        .any(
                            |&(ref opt_src, _)| if let &Rvalue::Constant {
                                       value, ..
                                   } = opt_src {
        ...
        if !bblock.is_empty() {
            ret.add_vertex(
                ControlFlowTarget::Resolved(BasicBlock::from_vec(bblock)),
            );
        }
        ...
        for (src_off, tgts) in by_source.iter() {
            for &(ref tgt, ref gu) in tgts {
                ...
                match (from_bb, to_bb) {
                    ...
                    (None, Some(to)) => {
                        if let Some(&ControlFlowTarget::Resolved(ref bb)) =
                            ret.vertex_label(to) {
                            if bb.area.start <= *src_off &&
                               bb.area.end > *src_off {
                                continue;
                            }
                        }

                        let vx = ret.add_vertex(
                            ControlFlowTarget::Unresolved(
                                Rvalue::new_u64(*src_off),
                            )
                        );
                    ...
                    (Some(from), None) => {
                        if let Some(&ControlFlowTarget::Resolved(ref bb)) =
                            ret.vertex_label(from) {
                            if let &Rvalue::Constant { value, .. } = tgt {
                                if bb.area.start <= value &&
                                   bb.area.end > value {
                                    continue;
                                }
                            }
                        ...
                        let vx =
                            ret.add_vertex(
                                ControlFlowTarget::Unresolved(tgt.clone()),
                            );
        ...
        A::Configuration: Debug,
    {
        ...
        let (mut mnemonics, mut by_source, mut by_destination) =
            cont.map_or(
        ...
        while let Some(addr) = todo.iter().next().cloned() {
            let maybe_mnes =
                mnemonics.iter().find(|x| *x.0 >= addr).map(|x| x.1.clone());
                        ...
                        Some(&MnemonicOrError::Mnemonic(ref mne)) => {
                            if mne.area.start < addr && mne.area.end > addr {
                                mnemonics
                                    ...
                                    .or_insert(Vec::new())
                                    .push(
                                        MnemonicOrError::Error(
                                            addr,
                                            "Jump inside instruction".into(),
                                        )
                                    );
                                continue;
                    ...
                    if match_st.mnemonics.is_empty() {
                        ...
                        for mne in match_st.mnemonics {
                            ...
                            mnemonics
                                .entry(mne.area.start)
                                .or_insert(Vec::new())
                                .push(MnemonicOrError::Mnemonic(mne));
                        }
                    }
                        ...
                        match tgt {
                            Rvalue::Constant { value: ref c, .. } => {
                                by_source
                                    .entry(origin)
                                    .or_insert(Vec::new())
                                    .push((tgt.clone(), gu.clone()));
                                by_destination
                                    .entry(*c)
                                    .or_insert(Vec::new())
                                    .push(
                                        (Rvalue::new_u64(origin), gu.clone()),
                                    );
                                todo.insert(*c);
                            }
                            _ => {
        ...
        let cfg = Self::assemble_cflow_graph(
            mnemonics,
            by_source,
            by_destination,
            start,
    ...
    pub fn collect_calls(&self) -> Vec<Rvalue> {
        ...
        for vx in self.cflow_graph.vertices() {
            if let Some(&ControlFlowTarget::Resolved(ref bb)) =
                self.cflow_graph.vertex_label(vx) {
                bb.execute(
                    |i| match i {
                        &Statement { op: Operation::Call(ref t), .. } => {
                            ret.push(t.clone())
                        }
                        _ => {}
                    }
...
mod tests {
    use super::*;
    use {Architecture, BasicBlock, Bound, Disassembler, Guard, Match,
         Mnemonic, OpaqueLayer, Region, Result, Rvalue, State};
    use panopticon_graph_algos::{AdjacencyMatrixGraphTrait,
                                 EdgeListGraphTrait, VertexListGraphTrait};
    ...
    fn index_resolved() {
        ...
        for e in cfg_re.edges() {
            if let Some(&ControlFlowTarget::Resolved(ref from)) =
                cfg_re.vertex_label(cfg_re.source(e)) {
                if let Some(&ControlFlowTarget::Resolved(ref to)) =
                    cfg_re.vertex_label(cfg_re.target(e)) {
                    assert!(
    ...
    fn index_unresolved() {
        ...
        let vx1 = cfg.add_vertex(ControlFlowTarget::Resolved(bb1));
        let vx2 =
            cfg.add_vertex(ControlFlowTarget::Unresolved(Rvalue::new_u32(42)));
        let vx3 =
            cfg.add_vertex(ControlFlowTarget::Unresolved(Rvalue::new_u32(23)));
    ...
    fn add_single() {
        ...
        if let Some(vx) = func.cflow_graph.vertices().next() {
            if let Some(&ControlFlowTarget::Resolved(ref bb)) =
                func.cflow_graph.vertex_label(vx) {
    ...
    fn continuous() {
        ...
        for vx in func.cflow_graph.vertices() {
            if let Some(&ControlFlowTarget::Resolved(ref bb)) =
                ...
                bb_vx = Some(vx);
            } else if let Some(&ControlFlowTarget::Failed(c, _)) =
                func.cflow_graph.vertex_label(vx) {
                assert_eq!(c, 6);
                ures_vx = Some(vx);
            } else {
        ...
        assert_eq!(func.name, "func_0".to_string());
        assert!(
            func.cflow_graph.edge(bb_vx.unwrap(), ures_vx.unwrap()).is_some()
        );
    }
    ...
    fn branch() {
        ...
        assert!(
            ures_vx.is_some() && bb0_vx.is_some() && bb1_vx.is_some() &&
            bb2_vx.is_some()
        );
        assert_eq!(func.entry_point, bb0_vx);
        assert_eq!(func.name, "func_0".to_string());
        assert!(
            func.cflow_graph.edge(bb0_vx.unwrap(), bb1_vx.unwrap()).is_some()
        );
        assert!(
            func.cflow_graph.edge(bb0_vx.unwrap(), bb2_vx.unwrap()).is_some()
        );
        assert!(
            func.cflow_graph.edge(bb1_vx.unwrap(), ures_vx.unwrap()).is_some()
        );
        assert!(
            func.cflow_graph.edge(bb2_vx.unwrap(), bb1_vx.unwrap()).is_some()
        );
    }
    ...
    #[test]
    fn entry_split() {
        let bb = BasicBlock::from_vec(
            vec![Mnemonic::dummy(0..1), Mnemonic::dummy(1..2)],
        );
        ...
        let vx1 = fun.cflow_graph.add_vertex(
            ControlFlowTarget::Unresolved(
                Rvalue::new_u32(2),
            )
        );
        ...
        let reg = Region::new("".to_string(), data);
        let func =
            Function::disassemble::<TestArchShort>(Some(fun), main, &reg, 2);
        ...
        assert!(
            func.cflow_graph.edge(bb1_vx.unwrap(), bb2_vx.unwrap()).is_some()
        );
        assert!(
            func.cflow_graph.edge(bb2_vx.unwrap(), bb1_vx.unwrap()).is_some()
        );
    ...
    fn issue_51_treat_entry_point_as_incoming_edge() {
        ...
        assert!(
            func.cflow_graph.edge(bb1_vx.unwrap(), bb0_vx.unwrap()).is_some()
        );
    }
    ...
    fn issue_232_overlap_with_entry_point() {
        ...
        assert_eq!(func.entry_point, bb1_vx);
        assert!(
            func.cflow_graph.edge(bb01_vx.unwrap(), bb2_vx.unwrap()).is_some()
        );
        assert!(
            func.cflow_graph.edge(bb1_vx.unwrap(), bb2_vx.unwrap()).is_some()
        );
        assert!(
            func.cflow_graph.edge(bb2_vx.unwrap(), bb01_vx.unwrap()).is_some()
        );
    }
}
