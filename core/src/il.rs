...
impl Display for Rvalue {
    fn fmt(&self, f: &mut Formatter) -> result::Result<(), Error> {
        match self {
            ...
            &Rvalue::Constant { value: v, size: s } => {
                f.write_fmt(format_args!("0x{:x}:{}", v, s))
            }
            &Rvalue::Variable {
                ref name,
                ref subscript,
    ...
    fn fmt(&self, f: &mut Formatter) -> result::Result<(), Error> {
        match self {
            ...
            &Guard::Predicate {
                ref flag,
                ref expected,
            } => f.write_fmt(format_args!("({} == {})", flag, expected)),
        }
    }
}
...
impl Statement {
    ...
    pub fn sanity_check(&self) -> Result<()> {
        ...
        let typecheck_binop = |a: &Rvalue,
                               b: &Rvalue,
                               assignee: &Lvalue|
         -> Result<()> {
            if !(a.size() == None || b.size() == None || a.size() == b.size()) {
                return Err(
                    format!("Argument sizes mismatch: {} vs. {}", a, b)
                        .into()
                );
            }

            if !(assignee.size() == None ||
                 Some(
                cmp::max(
                    a.size().unwrap_or(0),
                    b.size().unwrap_or(0),
                ...
                return Err(
                    "Operation result and assingnee sizes mismatch"
                        .into()
                );
            }
        ...
        let typecheck_cmpop =
            |a: &Rvalue, b: &Rvalue, assignee: &Lvalue| -> Result<()> {
                if !(a.size() == None || b.size() == None ||
                     a.size() == b.size()) {
        ...
        let typecheck_unop =
            |a: &Rvalue, sz: Option<usize>, assignee: &Lvalue| -> Result<()> {
                if sz.is_none() {
                    // zext?
                    if !(a.size() == None || assignee.size() == None ||
                         assignee.size() <= a.size()) {
                        return Err(
                            "Operation result and assingnee sizes mismatch"
                                .into()
                        );
        ...
        try!(
            match self {
                &Statement {
                    op: Operation::Add(ref a, ref b),
                    ref assignee,
                } => typecheck_binop(a, b, assignee),
                &Statement {
                    op: Operation::Subtract(ref a, ref b),
                    ref assignee,
                } => typecheck_binop(a, b, assignee),
                &Statement {
                    op: Operation::Multiply(ref a, ref b),
                    ref assignee,
                } => typecheck_binop(a, b, assignee),
                &Statement {
                    op: Operation::DivideUnsigned(ref a, ref b),
                    ref assignee,
                } => typecheck_binop(a, b, assignee),
                &Statement {
                    op: Operation::DivideSigned(ref a, ref b),
                    ref assignee,
                } => typecheck_binop(a, b, assignee),
                &Statement {
                    op: Operation::ShiftLeft(ref a, ref b),
                    ref assignee,
                } => typecheck_binop(a, b, assignee),
                &Statement {
                ...
                &Statement {
                    op: Operation::ShiftRightSigned(ref a, ref b),
                    ref assignee,
                } => typecheck_binop(a, b, assignee),
                &Statement {
                    op: Operation::Modulo(ref a, ref b),
                    ref assignee,
                } => typecheck_binop(a, b, assignee),
                &Statement {
                    op: Operation::And(ref a, ref b),
                    ref assignee,
                } => typecheck_binop(a, b, assignee),
                &Statement {
                    op: Operation::ExclusiveOr(ref a, ref b),
                    ref assignee,
                } => typecheck_binop(a, b, assignee),
                &Statement {
                    op: Operation::InclusiveOr(ref a, ref b),
                    ref assignee,
                } => typecheck_binop(a, b, assignee),

                &Statement {
                    op: Operation::Equal(ref a, ref b),
                    ref assignee,
                ...
                &Statement {
                    op: Operation::LessOrEqualSigned(ref a, ref b),
                    ref assignee,
                } => typecheck_cmpop(a, b, assignee),
                &Statement {
                    op: Operation::LessUnsigned(ref a, ref b),
                    ref assignee,
                } => typecheck_cmpop(a, b, assignee),
                &Statement {
                    op: Operation::LessSigned(ref a, ref b),
                    ref assignee,
                } => typecheck_cmpop(a, b, assignee),

                &Statement {
                    op: Operation::SignExtend(ref a, ref b),
                    ref assignee,
                } => typecheck_unop(b, Some(*a), assignee),
                &Statement {
                    op: Operation::ZeroExtend(ref a, ref b),
                    ref assignee,
                } => typecheck_unop(b, Some(*a), assignee),
                &Statement {
                    op: Operation::Move(ref a),
                    ref assignee,
                } => typecheck_unop(a, None, assignee),
                &Statement {
                    op: Operation::Select(ref off, ref a, ref b),
                    ref assignee,
                } => {
                    if !(assignee.size() == a.size() &&
                         *off + b.size().unwrap_or(0) <=
                         a.size().unwrap_or(0)) {
                ...
                &Statement {
                    ...
                    if !(assignee == &Lvalue::Undefined) {
                        return Err(
                            "Call operation can only be assigned to Undefined"
                                .into()
                        );
                    } else {
                        Ok(())
                ...
                &Statement {
                    op: Operation::Load(_, _),
                    ref assignee,
                } => {
                    if !(assignee.size().is_some()) {
                        return Err(
                            "Memory operation with undefined size"
                                .into()
                        );
                    } else {
                        Ok(())
                ...
                &Statement {
                    op: Operation::Phi(ref vec),
                    ref assignee,
                } => {
                    if !(vec.iter().all(
                        |rv| rv.size() == assignee.size(),
                    ) && assignee.size() != None) {
        ...
        if !(self.op.operands().iter().all(|rv| rv.size() != Some(0)) &&
             self.assignee.size() != Some(0)) {
...
pub fn execute(op: Operation<Rvalue>) -> Rvalue {
    match op {
        Operation::Add(Rvalue::Constant { value: _a, size: s },
                       Rvalue::Constant {
                           value: _b,
                           size: _s,
                       }) => {
            ...
            let b = Wrapping(_b);
            let mask =
                Wrapping(if s < 64 { (1u64 << s) - 1 } else { u64::MAX });
            Rvalue::Constant {
                value: ((a + b) & mask).0,
                size: s,
            }
        ...
        Operation::Subtract(Rvalue::Constant { value: _a, size: s },
                            Rvalue::Constant {
                                value: _b,
                                size: _s,
                            }) => {
        ...
        Operation::Multiply(Rvalue::Constant { value: _a, size: s },
                            Rvalue::Constant {
                                value: _b,
                                size: _s,
                            }) => {
        ...
        Operation::DivideUnsigned(Rvalue::Constant { value: _a, size: s },
                                  Rvalue::Constant {
                                      value: _b,
                                      size: _s,
                                  }) => {
        ...
        Operation::DivideUnsigned(Rvalue::Constant { value: 0, size: s },
                                  _) => Rvalue::Constant { value: 0, size: s },
        Operation::DivideUnsigned(_, _) => Rvalue::Undefined,

        Operation::DivideSigned(_, Rvalue::Constant { value: 0, .. }) => {
            Rvalue::Undefined
        }
        Operation::DivideSigned(Rvalue::Constant { value: _a, size: s },
                                Rvalue::Constant {
                                    value: _b,
                                    size: _s,
                                }) => {
        ...
        Operation::Modulo(Rvalue::Constant { value: a, size: s },
                          Rvalue::Constant { value: b, size: _s }) => {
            debug_assert!(s == _s);
        ...
        Operation::ShiftLeft(Rvalue::Constant { value: _a, size: s },
                             Rvalue::Constant { value: b, size: _s }) => {
            debug_assert!(s == _s);
        ...
        Operation::ShiftRightUnsigned(Rvalue::Constant {
                                          value: _a,
                                          size: s,
                                      },
                                      Rvalue::Constant {
        ...
        Operation::ShiftRightUnsigned(ref a,
                                      Rvalue::Constant {
                                          value: 0, ..
                                      }) => a.clone(),
        Operation::ShiftRightUnsigned(_, _) => Rvalue::Undefined,

        Operation::ShiftRightSigned(Rvalue::Constant {
                                        value: _a,
                                        size: s,
                                    },
                                    Rvalue::Constant {
        ...
        Operation::ShiftRightSigned(Rvalue::Constant { value: 0, size: s },
                                    _) => {
            Rvalue::Constant { value: 0, size: s }
        }
        Operation::ShiftRightSigned(ref a,
                                    Rvalue::Constant { value: 0, .. }) => {
            a.clone()
        }
        Operation::ShiftRightSigned(_, _) => Rvalue::Undefined,

        Operation::And(Rvalue::Constant { value: _a, size: s },
                       Rvalue::Constant {
                           value: _b,
                           size: _s,
                       }) => {
        ...
        Operation::InclusiveOr(Rvalue::Constant { value: _a, size: s },
                               Rvalue::Constant {
                                   value: _b,
                                   size: _s,
                               }) => {
        ...
        Operation::ExclusiveOr(Rvalue::Constant { value: _a, size: s },
                               Rvalue::Constant {
                                   value: _b,
                                   size: _s,
                               }) => {
        ...
        Operation::Equal(Rvalue::Constant { value: _a, size: s },
                         Rvalue::Constant {
                             value: _b,
                             size: _s,
                         }) => {
        ...
        Operation::LessOrEqualUnsigned(Rvalue::Constant {
                                           value: _a,
                                           size: s,
                                       },
                                       Rvalue::Constant {
        ...
        Operation::LessOrEqualSigned(Rvalue::Constant {
                                         value: _a,
                                         size: s,
                                     },
                                     Rvalue::Constant {
        ...
        Operation::LessUnsigned(Rvalue::Constant { value: _a, size: s },
                                Rvalue::Constant {
                                    value: _b,
                                    size: _s,
                                }) => {
        ...
        Operation::LessSigned(Rvalue::Constant { value: _a, size: s },
                              Rvalue::Constant {
                                  value: _b,
                                  size: _s,
                              }) => {
        ...
        Operation::Move(Rvalue::Constant {
                            ref value,
                            ref size,
                        }) => {
            if *size < 64 {
        ...
        Operation::Select(off,
                          Rvalue::Constant { value: _a, size: s },
                          Rvalue::Constant {
                              value: _b,
                              size: _s,
                          }) => {
...
pub fn lift<
    ...
    W: Clone + PartialEq + Eq + Debug + Encodable + Decodable,
    F: Fn(&V) -> W
>(
    op: &Operation<V>,
    m: &F,
    ...
    match op {
        ...
        &Operation::Load(ref s, _) => {
            Operation::Load(s.clone(), args[0].clone())
        }
        &Operation::Store(ref s, _) => {
            Operation::Store(s.clone(), args[0].clone())
        }
        &Operation::Add(_, _) => {
            Operation::Add(args[0].clone(), args[1].clone())
        }
        &Operation::Subtract(_, _) => {
            Operation::Subtract(args[0].clone(), args[1].clone())
        }
        &Operation::Multiply(_, _) => {
            Operation::Multiply(args[0].clone(), args[1].clone())
        }
        &Operation::DivideUnsigned(_, _) => {
            Operation::DivideUnsigned(args[0].clone(), args[1].clone())
        }
        &Operation::DivideSigned(_, _) => {
            Operation::DivideSigned(args[0].clone(), args[1].clone())
        }
        &Operation::ShiftLeft(_, _) => {
            Operation::ShiftLeft(args[0].clone(), args[1].clone())
        }
        &Operation::ShiftRightUnsigned(_, _) => {
            Operation::ShiftRightUnsigned(args[0].clone(), args[1].clone())
        }
        &Operation::ShiftRightSigned(_, _) => {
            Operation::ShiftRightSigned(args[0].clone(), args[1].clone())
        }
        &Operation::Modulo(_, _) => {
            Operation::Modulo(args[0].clone(), args[1].clone())
        }
        &Operation::And(_, _) => {
            Operation::And(args[0].clone(), args[1].clone())
        }
        &Operation::InclusiveOr(_, _) => {
            Operation::InclusiveOr(args[0].clone(), args[1].clone())
        }
        &Operation::ExclusiveOr(_, _) => {
            Operation::ExclusiveOr(args[0].clone(), args[1].clone())
        }
        &Operation::Equal(_, _) => {
            Operation::Equal(args[0].clone(), args[1].clone())
        }
        &Operation::LessUnsigned(_, _) => {
            Operation::LessUnsigned(args[0].clone(), args[1].clone())
        }
        &Operation::LessSigned(_, _) => {
            Operation::LessSigned(args[0].clone(), args[1].clone())
        }
        &Operation::LessOrEqualUnsigned(_, _) => {
            Operation::LessOrEqualUnsigned(args[0].clone(), args[1].clone())
        }
        &Operation::LessOrEqualSigned(_, _) => {
            Operation::LessOrEqualSigned(args[0].clone(), args[1].clone())
        }
        &Operation::Call(_) => Operation::Call(args[0].clone()),
        &Operation::Move(_) => Operation::Move(args[0].clone()),
        &Operation::Select(ref off, _, _) => {
            Operation::Select(*off, args[0].clone(), args[1].clone())
        }
        &Operation::ZeroExtend(ref sz, _) => {
            Operation::ZeroExtend(*sz, args[0].clone())
        }
        &Operation::SignExtend(ref sz, _) => {
            Operation::SignExtend(*sz, args[0].clone())
        }
    }
}
    ...
    fn fmt(&self, f: &mut Formatter) -> result::Result<(), Error> {
        match self.op {
            Operation::Add(ref a, ref b) => {
                f.write_fmt(format_args!("add {}, {}, {}", self.assignee, a, b))
            }
            Operation::Subtract(ref a, ref b) => {
                f.write_fmt(format_args!("sub {}, {}, {}", self.assignee, a, b))
            }
            Operation::Multiply(ref a, ref b) => {
                f.write_fmt(format_args!("mul {}, {}, {}", self.assignee, a, b))
            }
            Operation::DivideUnsigned(ref a, ref b) => {
                f.write_fmt(
                    format_args!("divu {}, {}, {}", self.assignee, a, b),
                )
            }
            Operation::DivideSigned(ref a, ref b) => {
                f.write_fmt(
                    format_args!("divs {}, {}, {}", self.assignee, a, b),
                )
            }
            Operation::ShiftLeft(ref a, ref b) => {
                f.write_fmt(format_args!("shl {}, {}, {}", self.assignee, a, b))
            }
            Operation::ShiftRightUnsigned(ref a, ref b) => {
                f.write_fmt(
                    format_args!("shru {}, {}, {}", self.assignee, a, b),
                )
            }
            Operation::ShiftRightSigned(ref a, ref b) => {
                f.write_fmt(
                    format_args!("shrs {}, {}, {}", self.assignee, a, b),
                )
            }
            Operation::Modulo(ref a, ref b) => {
                f.write_fmt(format_args!("mod {}, {}, {}", self.assignee, a, b))
            }
            Operation::And(ref a, ref b) => {
                f.write_fmt(format_args!("and {}, {}, {}", self.assignee, a, b))
            }
            Operation::InclusiveOr(ref a, ref b) => {
                f.write_fmt(format_args!("or {}, {}, {}", self.assignee, a, b))
            }
            Operation::ExclusiveOr(ref a, ref b) => {
                f.write_fmt(format_args!("xor {}, {}, {}", self.assignee, a, b))
            }

            Operation::Equal(ref a, ref b) => {
                f.write_fmt(
                    format_args!("cmpeq {}, {}, {}", self.assignee, a, b),
                )
            }
            Operation::LessOrEqualUnsigned(ref a, ref b) => {
                f.write_fmt(
                    format_args!("cmpleu {}, {}, {}", self.assignee, a, b),
                )
            }
            Operation::LessOrEqualSigned(ref a, ref b) => {
                f.write_fmt(
                    format_args!("cmples {}, {}, {}", self.assignee, a, b),
                )
            }
            Operation::LessUnsigned(ref a, ref b) => {
                f.write_fmt(
                    format_args!("cmplu {}, {}, {}", self.assignee, a, b),
                )
            }
            Operation::LessSigned(ref a, ref b) => {
                f.write_fmt(
                    format_args!("cmpls {}, {}, {}", self.assignee, a, b),
                )
            }

            Operation::ZeroExtend(s, ref a) => {
                f.write_fmt(
                    format_args!("convert_{} {}, {}", s, self.assignee, a),
                )
            }
            Operation::SignExtend(s, ref a) => {
                f.write_fmt(
                    format_args!("sign-extend_{} {}, {}", s, self.assignee, a),
                )
            }
            ...
            Operation::Move(ref a) => {
                f.write_fmt(format_args!("mov {}, {}", self.assignee, a))
            }
            Operation::Call(ref a) => {
                f.write_fmt(format_args!("call {}, {}", self.assignee, a))
            }

            Operation::Load(ref r, ref b) => {
                f.write_fmt(format_args!("load_{} {}, {}", r, self.assignee, b))
            }
            Operation::Store(ref r, ref b) => {
                f.write_fmt(
                    format_args!("store_{} {}, {}", r, self.assignee, b),
                )
            }
...
impl Arbitrary for Operation<Rvalue> {
    fn arbitrary<G: Gen>(g: &mut G) -> Self {
        let mut op = match g.gen_range(0, 24) {
            ...
            11 => {
                Operation::Equal(Rvalue::arbitrary(g), Rvalue::arbitrary(g))
            }
            12 => {
...
mod tests {
    ...
    fn setup() -> Vec<Statement> {
        vec![
            ...
            Statement {
                op: Operation::ShiftRightUnsigned(
                    Rvalue::Undefined,
                    Rvalue::Undefined,
                ),
                assignee: Lvalue::Undefined,
            ...
            Statement {
                op: Operation::LessOrEqualSigned(
                    Rvalue::Undefined,
                    Rvalue::Undefined,
                ),
                assignee: Lvalue::Undefined,
