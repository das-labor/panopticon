...
fn load_elf(bytes: &[u8], name: String) -> Result<(Project, Machine)> {
    ...
    for ph in &binary.program_headers {
        if ph.p_type == program_header::PT_LOAD {
            ...
            if cursor.seek(SeekFrom::Start(ph.p_offset)).ok() ==
               Some(ph.p_offset) {
        ...
        if sym.is_function() {
            if sym.is_import() {
                prog.call_graph.add_vertex(
                    CallTarget::Symbolic(name, Uuid::new_v4()),
                );
            } else {
                prog.call_graph
                    .add_vertex(
    ...
    let resolve_import_address = |proj: &mut Project,
                                  relocs: &[elf::Reloc],
                                  name: &str| {
        for reloc in relocs {
...
fn load_pe(bytes: &[u8], name: String) -> Result<(Project, Machine)> {
    ...
    for import in pe.imports {
        ...
        prog.call_graph.add_vertex(
            CallTarget::Symbolic(import.name, Uuid::new_v4()),
        );
    }
