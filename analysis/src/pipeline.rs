...
use panopticon_core::{Architecture, CallTarget, Function, Program, Region,
                      Rvalue};
    ...
    A::Configuration: Debug,
{
    ...
    thread::spawn(
        move || {
            ...
            let mut targets =
                HashMap::<u64, Function>::from_iter(
                    program
                        .call_graph
                        .vertices()
                        .filter_map(
                            |vx| match program.call_graph.vertex_label(vx) {
                                Some(&CallTarget::Todo(Rvalue::Constant {
                                                          value: entry, ..
                                                      },
                                                      ref maybe_name,
                                                      ref uuid)) => {
                                    let name = maybe_name
                                        .clone()
                                        .unwrap_or_else(
                                            || format!("func_0x{:x}", entry),
                                        );
                                    let f = Function::with_uuid(
                                        name,
                                        uuid.clone(),
                                        region.name().clone(),
                                    );
            ...
            while !targets.is_empty() {
                ...
                let new_targets: Vec<Vec<(u64, Function)>> = targets
                    ...
                    .map(
                        |(entry, f)| {
                            let tx = tx.clone();
                            let mut f = Function::disassemble::<A>(
                                Some(f),
                                config.clone(),
                                &region,
                                entry,
                            ...
                            tx.send_all(stream::iter(vec![Ok(f)]))
                                .wait()
                                .unwrap()
                                .0;
                            new_ct
