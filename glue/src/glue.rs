...
use errors::*;
use ffi::{start_gui_loop, update_current_session, update_function_edges,
          update_function_node, update_layout_task, update_sidebar_items,
          update_undo_redo};
...
pub trait Glue {
    ...
    fn exec(
        ...
        recent_sessions: Vec<(String, String, PathBuf, u32)>,
    ) -> Result<()> {
        ...
        let recent_sessions = recent_sessions
            .into_iter()
            .filter_map(|x| CRecentSession::new(x.0, x.1, &x.2, x.3).ok())
            .collect::<Vec<_>>();
        let mut recent_sess_ptrs: Vec<*const CRecentSession> = recent_sessions
            .iter()
            .map(|x| -> *const CRecentSession { x })
            .collect();
    ...
    fn update_sidebar(funcs: &[Function]) {
        let items = funcs
            .iter()
            .filter_map(|f| CSidebarItem::new(f).ok())
            .collect::<Vec<_>>();
        let mut ptrs: Vec<*const CSidebarItem> =
            items.iter().map(|i| -> *const CSidebarItem { i }).collect();
    ...
    fn send_function_node(
        uuid: CString,
        id: usize,
        x: f32,
        y: f32,
        is_entry: bool,
        lines: &[CBasicBlockLine],
    ) -> Result<()> {
        let mut ptrs: Vec<*const CBasicBlockLine> =
            lines.iter().map(|i| -> *const CBasicBlockLine { i }).collect();
    ...
    fn send_function_edges(
        ...
        let mut label_ptrs: Vec<*const i8> =
            labels.iter().map(|i| -> *const i8 { i.as_ptr() }).collect();
        let mut kind_ptrs: Vec<*const i8> =
            kinds.iter().map(|i| -> *const i8 { i.as_ptr() }).collect();
    ...
    extern "C" fn get_function_plumbing(
        uuid_cstr: *const i8,
        only_entry: i8,
        do_nodes: i8,
        do_edges: i8,
        ...
        let uuid =
            unsafe { CStr::from_ptr(uuid_cstr) }.to_string_lossy().to_string();
    ...
    extern "C" fn subscribe_to_plumbing(
        uuid_cstr: *const i8,
        state: i8,
    ) -> i32 {
        let uuid =
    ...
    extern "C" fn comment_on_plumbing(address: u64, comment: *const i8) -> i32 {
        let comment =
            unsafe { CStr::from_ptr(comment) }.to_string_lossy().to_string();
    ...
    extern "C" fn rename_function_plumbing(
        uuid: *const i8,
        name: *const i8,
    ) -> i32 {
        let name =
        ...
        let uuid =
            unsafe { CStr::from_ptr(uuid) }.to_string_lossy().to_string();
        let uuid = match Uuid::parse_str(&uuid) {
            Ok(uuid) => uuid,
            Err(s) => {
    ...
    extern "C" fn set_value_for_plumbing(
        uuid: *const i8,
        variable: *const i8,
        value: *const i8,
    ) -> i32 {
        let uuid =
            unsafe { CStr::from_ptr(uuid) }.to_string_lossy().to_string();
        let variable =
            unsafe { CStr::from_ptr(variable) }.to_string_lossy().to_string();
        let value =
