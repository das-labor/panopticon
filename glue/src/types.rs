...
impl CBasicBlockLine {
    pub fn new(
        ...
        offset: u64,
        comment: String,
        args: Vec<CBasicBlockOperand>,
    ) -> Result<CBasicBlockLine> {
        ...
        let mut args: Vec<*const CBasicBlockOperand> = args.into_iter()
            .map(
                |i| -> *const CBasicBlockOperand { Box::into_raw(Box::new(i)) },
            )
            .collect();
