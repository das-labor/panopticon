...
use panopticon_core::{Lvalue, Result, Rvalue, Statement,
                 execute, lift};
use panopticon_amd64::{AddressingMethod, JumpSpec, MnemonicSpec, Opcode,
                        Operand, OperandSpec, OperandType, read_spec_register,
                        semantic, tables};
...
fn sample_simd_register<G: Gen>(
    g: &mut G,
    opsz: usize,
) -> Result<SampledOperand> {
    Ok(
        match opsz {
            32 => {
                SampledOperand::Register(
...
fn sample_register_variant<G: Gen>(
    reg: &OperandType,
    opsz: usize,
    g: &mut G,
) -> Result<SampledOperand> {
...
fn sample_operand<G: Gen>(
    ...
    match (spec, opsz) {
        (&OperandSpec(AddressingMethod::None, ref reg), opsz) => {
        ...
        (&OperandSpec(AddressingMethod::E, OperandType::z), opsz) => {
        ...
        (&OperandSpec(AddressingMethod::I, OperandType::one),
         opsz) => Ok(SampledOperand::Immediate(1, opsz)),
...
fn operand_specs(mnemonic: &'static str) -> Vec<Vec<&'static OperandSpec>> {
    ...
    fn _impl(
        cell: &'static Opcode,
        mnemonic: &'static str,
        ret: &mut Vec<Vec<&'static OperandSpec>>,
    ) {
        match cell {
            &Opcode::Nonary(MnemonicSpec::Single(ref mne), _, _)
                if *mne == mnemonic => ret.push(vec![]),
            &Opcode::Unary(MnemonicSpec::Single(ref mne), _, _, ref op)
                if *mne == mnemonic => ret.push(vec![op]),
            &Opcode::Binary(MnemonicSpec::Single(ref mne),
                            _,
                            _,
                            ref op1,
                            ref op2) if *mne == mnemonic => {
                ret.push(vec![op1, op2])
            }
            &Opcode::Trinary(MnemonicSpec::Single(ref mne),
                             _,
                             _,
                             ref op1,
                             ref op2,
                             ref op3) if *mne == mnemonic => {
                ret.push(vec![op1, op2, op3])
            }
            &Opcode::Quaternary(MnemonicSpec::Single(ref mne),
                                _,
                                _,
                                ref op1,
                                ref op2,
                                ref op3,
                                ref op4) if *mne == mnemonic => {
...
fn rappel_xcheck(
    ...
    match (&a, &b) {
        (&SampledOperand::Register(ref nam1, ref val1, ref sz1),
         &SampledOperand::Register(ref nam2, ref val2, ref sz2)) => {
        ...
        (&SampledOperand::Register(ref nam1, ref val1, ref sz1),
         &SampledOperand::Immediate(ref val2, ref sz2)) => {
            let a_var = Rvalue::Variable {
                name: nam1.clone().into(),
                size: *sz1,
                subscript: None,
    ...
    if let (&mut Some(ref mut stdin), &Some(_)) =
        (&mut child.stdin, &child.stdout) {
        let mov = &|nam: &str,
                    val: u64,
                    sz: usize,
                    stdin: &mut Write|
