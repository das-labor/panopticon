...
fn amd64_opcodes() {
    let reg =
        Region::open("com".to_string(), Path::new("../tests/data/amd64.com"))
            .unwrap();
    let mut addr = 0;

    loop {
        let maybe_match = <amd64::Amd64 as Architecture>::decode(
            &reg,
            addr,
            &amd64::Mode::Long,
        );
...
fn ia32_opcodes() {
    ...
    let reg = Region::open("com".to_string(), Path::new("../tests/data/ia32.com"))
        .unwrap();
    let mut addr = 0;

    loop {
        let maybe_match =
            amd64::Amd64::decode(&reg, addr, &amd64::Mode::Protected);
