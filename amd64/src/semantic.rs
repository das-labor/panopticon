...
fn sign_extend(
    a: &Rvalue,
    b: &Rvalue,
) -> Result<(Rvalue, Rvalue, usize, Vec<Statement>)> {
    extend(a, b, true)
}
...
fn extend(
    ...
    assert!(sz > 0);
    assert!(
        ext_a.size() == None || ext_b.size() == None ||
        ext_a.size() == ext_b.size()
    );
...
fn reg_variants(name: &str)
    -> Option<(Lvalue, Lvalue, Lvalue, Lvalue, Lvalue)> {
    match name {
        "AL" | "AH" | "AX" | "EAX" | "RAX" => {
            Some(
                (rreil_lvalue!{ AL:8 },
    ...
    if let &Rvalue::Variable {
        ...
        if let Some((reg8l, reg8h, reg16, reg32, reg64)) = reg_variants(name) {
            ...
            if hi >= 9 && lo <= 15 && reg8h != Lvalue::Undefined {
                let msk = (!((0xffff << lo) % (1u64 << cmp::min(16, hi))) &
                           0xffff) >> 8;
                if msk == 0 {
                    stmts.append(
                        &mut try!(
            ...
            // E*X
            if lo <= 31 {
                let msk = !((0xffffffff << lo) % (1u64 << cmp::min(32, hi))) &
                          0xffffffff;
...
pub fn jcc(a: Rvalue, cond: Condition) -> Result<(Vec<Statement>, JumpSpec)> {
    let g = match cond {
        ...
        Condition::NotOverflow => {
            Guard::from_flag(&rreil_rvalue!{ OF:1 })?.negation()
        }
        //Condition::Below => Guard::from_flag(rreil_rvalue!{ CF:1 }),
        Condition::AboveEqual => {
            Guard::from_flag(&rreil_rvalue!{ CF:1 })?.negation()
        }
        Condition::Equal => Guard::from_flag(&rreil_rvalue!{ ZF:1 })?,
        Condition::NotEqual => {
            Guard::from_flag(&rreil_rvalue!{ ZF:1 })?.negation()
        }
        ...
        Condition::NotSign => {
            Guard::from_flag(&rreil_rvalue!{ SF:1 })?.negation()
        }
        Condition::Parity => Guard::from_flag(&rreil_rvalue!{ PF:1 })?,
        Condition::NotParity => {
            Guard::from_flag(&rreil_rvalue!{ PF:1 })?.negation()
        }
