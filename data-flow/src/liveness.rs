...
use panopticon_core::{ControlFlowRef, ControlFlowTarget, Function, Guard,
                      Lvalue, Operation, Rvalue, Statement};
...
pub fn liveness_sets(func: &Function)
    -> (HashMap<ControlFlowRef, HashSet<Cow<'static, str>>>,
        HashMap<ControlFlowRef, HashSet<Cow<'static, str>>>) {
    let mut uevar = HashMap::<ControlFlowRef, HashSet<&str>>::new();
    let mut varkill =
        HashMap::<ControlFlowRef, HashSet<Cow<'static, str>>>::new();
    ...
    for &vx in ord.iter() {
        ...
        let vk =
            varkill.entry(vx).or_insert(HashSet::<Cow<'static, str>>::new());
        ...
        uevar
            ...
            .map(
                |(&k, v)| {
                    (k,
                     HashSet::from_iter(
                        v.iter().map(|x| Cow::Owned(x.to_string())),
                    ))
                }
            )
    ))
...
pub fn liveness(func: &Function)
    ...
    while !fixpoint {
        ...
        for &vx in ord.iter() {
            ...
            if let Some(&ControlFlowTarget::Resolved(_)) =
                cfg.vertex_label(vx) {
                for e in cfg.out_edges(vx) {
                    let m = cfg.target(e);
...
mod tests {
    ...
    fn live() {
        ...
        let all =
            HashSet::from_iter(vec![Cow::Borrowed("i"), Cow::Borrowed("s")]);
        ...
        assert_eq!(
            ...
            Some(
                &HashSet::from_iter(
                    vec![Cow::Borrowed("i"), Cow::Borrowed("s")],
                )
            )
        );
        assert_eq!(
