...
//use panopticon_core::il::{Statement, Operation, RREIL};
use panopticon_core::il::{RREIL};
use panopticon_core::il::neo::{Statement, Operation};
use panopticon_core::{self, Language, Function, Result, CfgNode, Mnemonic, BasicBlockIndex, Value, Variable};

use liveness::{Liveness,Globals};

pub fn phi_functions<'a, IL: Language>(func: &'a Function<IL>, globals: &Globals<'a>, domfronts: &Vec<BitSet>) -> Result<Vec<BitSet>> {
...
fn dominance_frontiers<IL: Language>(func: &Function<IL>, doms: &Dominators<NodeIndex>) -> Result<Vec<BitSet>> {
    let cfg = func.cfg();
...
fn fix_uninitialized_variables_rreil(basic_block: &mut Vec<(Mnemonic, Vec<panopticon_core::Statement>)>, uninit: BitSet, variables: &Vec<Variable>) {
    use panopticon_core::{Statement, Lvalue, Operation};
    basic_block.retain(|&(ref mne,_)| mne.opcode != "__init");
    let start = basic_block[0].0.area.start;
    for bit in uninit.iter() {
        let var = &variables[bit];
            ...
            DomTreeEvent::Enter{ index: bb_idx, start, successors, num_in_edges } => {
                ...
                for &mut (ref mut mnemonic,ref mut stmts) in basic_blocks[bb_idx.index()].iter_mut() {
                    for stmt in stmts.iter_mut() {
                        use panopticon_core::il::neo::Statement::*;
                ...
                'outer: for succ in successors {
                    ...
                    for &mut (ref mne,ref mut stmts) in succ_bb.iter_mut() {
                        ...
                        for stmt in stmts.iter_mut() {
                            use panopticon_core::il::neo::Statement::*;
                            use panopticon_core::il::neo::Operation::Phi;
...
fn dominator_tree<IL: Language>(func: &Function<IL>, doms: &Dominators<NodeIndex>) -> Result<Vec<DomTreeEvent>> {
    let num_bb = func.basic_blocks().len();
    let cfg = func.cfg();
...
pub fn rewrite_to_ssa(func: &mut Function) -> Result<()> {
    ...
    let doms = dominators::simple_fast(func.cfg(), func.basic_block(entry).node);
...
pub fn rewrite_to_ssa_rreil(func: &mut Function<RREIL>) -> Result<()> {
    let entry = func.entry_point();
    let doms = dominators::simple_fast(func.cfg(), func.basic_block(entry).node);
    let dom_events = dominator_tree(func,&doms)?;
    let (phis,uninit,variables) = {
...
mod tests {
    ...
    use panopticon_core::{Function,Variable};
    use liveness::{Liveness,Globals};
    use env_logger;
    ...
    fn dom_frontiers() {
        ...
        let doms = dominators::simple_fast(func.cfg(), func.basic_block(ent_idx).node);
        let df = dominance_frontiers(&func,&doms).unwrap();
