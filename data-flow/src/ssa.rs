...
use liveness_sets;
use panopticon_core::{ControlFlowEdge, ControlFlowGraph, ControlFlowRef,
                      ControlFlowTarget, Function, Guard, Lvalue, Mnemonic,
                      Operation, Result, Rvalue, Statement};
use panopticon_graph_algos::{BidirectionalGraphTrait, EdgeListGraphTrait,
                             GraphTrait, IncidenceGraphTrait,
                             MutableGraphTrait, VertexListGraphTrait};
use panopticon_graph_algos::dominator::{dominance_frontiers,
                                        immediate_dominator};
use std::borrow::Cow;
use std::cmp::max;
...
pub fn type_check(func: &Function)
    -> Result<HashMap<Cow<'static, str>, usize>> {
...
pub fn global_names(func: &Function)
    -> (HashSet<Cow<'static, str>>,
        HashMap<Cow<'static, str>, HashSet<ControlFlowRef>>) {
    let (varkill, uevar) = liveness_sets(func);
    let mut usage =
        HashMap::<Cow<'static, str>, HashSet<ControlFlowRef>>::new();
...
pub fn phi_functions(func: &mut Function) -> Result<()> {
    ...
    if let Some(&mut ControlFlowTarget::Resolved(ref mut bb)) =
        cfg.vertex_label_mut(func.entry_point.unwrap()) {
                ...
                |nam| {
                    let len =
                        try!(
                            lens.get(nam)
                                .ok_or(
                                    format!("No length for variable {}", nam),
                                )
                        );
        ...
        let instrs =
            instrs.into_iter().map(|x| x.ok().unwrap()).collect::<Vec<_>>();
    ...
    if idom.len() != cfg.num_vertices() {
        return Err(
            "No all basic blocks are reachable from function entry point"
                .into()
        );
    }
    ...
    for v in globals.iter() {
        ...
        while !worklist.is_empty() {
            ...
            let frontiers =
                try!(df.get(&w).ok_or("Incomplete dominance frontier set"));

            worklist.remove(&w);
            for d in frontiers.iter() {
                let arg_num = cfg.in_edges(*d)
                    .filter_map(
                        |p| usage.get(v).map(|b| b.contains(&cfg.source(p))),
                    )
                    .count();
                if let Some(&mut ControlFlowTarget::Resolved(ref mut bb)) =
                    cfg.vertex_label_mut(*d) {
                    if !phis.contains(&(v, *d)) {
                        ...
                        let len =
                            try!(
                                lens.get(v)
                                    .ok_or(
                                        format!("No length for variable {}", v),
                                    )
                            );
                        let mne = Mnemonic::new(
                            pos..pos,
...
pub fn rename_variables(func: &mut Function) -> Result<()> {
    ...
    let mut cfg = &mut func.cflow_graph;
    let mut stack = HashMap::<Cow<'static, str>, Vec<usize>>::from_iter(
        globals.iter().map(|x| (x.clone(), Vec::new())),
    );
    ...
    fn rename(
            ...
            for mne in bb.mnemonics.iter_mut() {
                if mne.opcode != "__phi" {
                    for o in mne.operands.iter_mut() {
                        if let &mut Rvalue::Variable {
                                        ref name,
                                        ref mut subscript,
                                        ..
                                    } = o {
                            *subscript =
                                stack.get(name).and_then(|x| x.last()).cloned();
                        }
                    }
                    ...
                    for i in mne.instructions.iter_mut() {
                        ...
                        if let &mut Operation::Phi(_) = op {
                            return Err(
                                "Phi instruction outside __phi mnemonic"
                                    .into()
                            );
                        } else {
                            ...
                            if let &mut Lvalue::Variable {
                                            ref name,
                                            ref mut subscript,
                                            ..
                                        } = assignee {
                                *subscript =
                                    Some(new_name(name, counter, stack));
                            }
                        }
                    }
                }
        ...
        for s in succ {
            if let Some(&mut Guard::Predicate {
                                 flag: Rvalue::Variable {
                                     ref name,
                                     ref mut subscript,
                                     ..
                                 },
                                 ..
                             }) = cfg.edge_label_mut(s) {
                *subscript = stack[name].last().cloned();
            }
        ...
        if let Some(&mut ControlFlowTarget::Resolved(ref mut bb)) =
            ...
            bb.execute(
                |i| match i {
                    &Statement {
                        assignee: Lvalue::Variable { ref name, .. }, ..
                    } => {
...
/// analysis.
pub fn flag_operations(func: &Function)
    -> HashMap<ControlFlowEdge, Operation<Rvalue>> {
    let mut ret = HashMap::new();

    for e in func.cflow_graph.edges() {
        if !ret.contains_key(&e) {
            if let Some(&Guard::Predicate { ref flag, .. }) =
                func.cflow_graph.edge_label(e) {
                let maybe_bb =
                    func.cflow_graph.vertex_label(func.cflow_graph.source(e));
...
mod tests {
    ...
    use panopticon_core::{BasicBlock, ControlFlowGraph, ControlFlowTarget,
                          Function, Guard, Lvalue, Mnemonic, Operation,
                          Rvalue, Statement};
    use std::borrow::Cow;
    ...
    #[test]
    fn phi() {
        let a = Lvalue::Variable {
            name: Cow::Borrowed("a"),
            size: 32,
            subscript: None,
        };
        let b = Lvalue::Variable {
            name: Cow::Borrowed("b"),
            size: 32,
            subscript: None,
        };
        let c = Lvalue::Variable {
            name: Cow::Borrowed("c"),
            size: 32,
            subscript: None,
        };
        let d = Lvalue::Variable {
            name: Cow::Borrowed("d"),
            size: 32,
            subscript: None,
        };
        let y = Lvalue::Variable {
            name: Cow::Borrowed("y"),
            size: 32,
            subscript: None,
        };
        let z = Lvalue::Variable {
            name: Cow::Borrowed("z"),
            size: 32,
            subscript: None,
        };
        ...
        let a0 = Lvalue::Variable {
            name: Cow::Borrowed("a"),
            size: 32,
            subscript: None,
        };
        let b0 = Lvalue::Variable {
            name: Cow::Borrowed("b"),
            size: 32,
            subscript: None,
        };
        let c0 = Lvalue::Variable {
            name: Cow::Borrowed("c"),
            size: 32,
            subscript: None,
        };
        let d0 = Lvalue::Variable {
            name: Cow::Borrowed("d"),
            size: 32,
            subscript: None,
        };
        let i0 = Lvalue::Variable {
            name: Cow::Borrowed("i"),
            size: 32,
            subscript: None,
        };
        ...
        // bb0
        if let Some(&ControlFlowTarget::Resolved(ref bb)) =
            func.cflow_graph.vertex_label(v0) {
        ...
        // bb1
        if let Some(&ControlFlowTarget::Resolved(ref bb)) =
            func.cflow_graph.vertex_label(v1) {
        ...
        // bb2
        if let Some(&ControlFlowTarget::Resolved(ref bb)) =
            func.cflow_graph.vertex_label(v2) {
            assert!(bb.mnemonics[0].opcode != "__phi".to_string());
        } else {
            unreachable!()
        }
        ...
        // bb3
        if let Some(&ControlFlowTarget::Resolved(ref bb)) =
            func.cflow_graph.vertex_label(v3) {
            assert_eq!(bb.mnemonics[0].opcode, "__phi".to_string());
            assert_eq!(bb.mnemonics[1].opcode, "__phi".to_string());
            assert_eq!(bb.mnemonics[2].opcode, "__phi".to_string());
            assert_eq!(bb.mnemonics[3].opcode, "__phi".to_string());
        ...
        // bb4
        if let Some(&ControlFlowTarget::Resolved(ref bb)) =
            func.cflow_graph.vertex_label(v4) {
            assert!(bb.mnemonics[0].opcode != "__phi".to_string());
        } else {
            unreachable!()
        }
        ...
        // bb5
        if let Some(&ControlFlowTarget::Resolved(ref bb)) =
            func.cflow_graph.vertex_label(v5) {
            assert!(bb.mnemonics[0].opcode != "__phi".to_string());
        } else {
            unreachable!()
        }
        ...
        // bb6
        if let Some(&ControlFlowTarget::Resolved(ref bb)) =
            func.cflow_graph.vertex_label(v6) {
            assert!(bb.mnemonics[0].opcode != "__phi".to_string());
        } else {
            unreachable!()
        }
        ...
        // bb7
        if let Some(&ControlFlowTarget::Resolved(ref bb)) =
            func.cflow_graph.vertex_label(v7) {
            assert_eq!(bb.mnemonics[0].opcode, "__phi".to_string());
            assert_eq!(bb.mnemonics[1].opcode, "__phi".to_string());
    ...
    fn rename() {
        ...
        for v in func.cflow_graph.vertices() {
            if let Some(&ControlFlowTarget::Resolved(ref bb)) =
                func.cflow_graph.vertex_label(v) {
                bb.execute(
                    |i| {
