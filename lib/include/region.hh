...
namespace po
{
	...
	struct layer
	{
		...
		layer(const std::string&, std::initializer_list<const byte>);
		layer(const std::string&, const std::vector<const byte>&);
	...
	private:
		...
		boost::variant<
			...
			std::vector<const byte>,					///< Constant data. Ignores Input.
