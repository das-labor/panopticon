...
void po::save_point(rdf::storage &store)
{
	...
	for(const std::pair<uuid,std::pair<marshal_poly,marshal_poly>> &p: dirty_locations)
	{
		...
		std::for_each(to_del.cbegin(),to_del.cend(),std::bind(&rdf::storage::remove,&store,std::placeholders::_1));

		rdf::statements to_new = p.second.second();
		std::for_each(to_new.cbegin(),to_new.cend(),std::bind((void(rdf::storage::*)(const rdf::statement&))&rdf::storage::insert,&store,std::placeholders::_1));
	}
