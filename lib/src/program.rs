...
use graph_algos::{AdjacencyList, GraphTrait, MutableGraphTrait, AdjacencyMatrixGraphTrait, VertexListGraphTrait};
...
impl Program {
    ...
    pub fn find_function_by_uuid<'a>(&'a self, a: &Uuid) -> Option<&'a Function> {
        self.call_graph
            .vertices()
            .find(|&x| match self.call_graph.vertex_label(x) {
    ...
    pub fn insert(&mut self, new_ct: CallTarget) -> Vec<Uuid> {
        ...
        let maybe_vx = self.call_graph
            .vertices()
            .find(|ct| self.call_graph.vertex_label(*ct).unwrap().uuid() == new_uu);
...
mod tests {
    use super::*;
    use uuid::Uuid;
    use graph_algos::{VertexListGraphTrait, GraphTrait, MutableGraphTrait, AdjacencyMatrixGraphTrait, EdgeListGraphTrait};
    use {ControlFlowTarget, Function, Mnemonic, BasicBlock, Lvalue, Rvalue, Operation, Statement};
    ...
    fn find_by_entry() {
        ...
        prog.call_graph
            .add_vertex(CallTarget::Concrete(Function::new("test".to_string(), "ram".to_string())));
    ...
    fn insert_replaces_todo() {
        ...
        let vx1 = prog.call_graph
            .add_vertex(CallTarget::Concrete(Function::new("test2".to_string(), "ram".to_string())));
