...
use {Program, CallTarget, Project, Layer, Region, Bound, Rvalue, Result};
...
fn load_elf(fd: &mut File, name: String) -> Result<(Project, Machine)> {
    ...
    prog.call_graph
        .add_vertex(CallTarget::Todo(Rvalue::new_u64(entry as u64), Some(name), Uuid::new_v4()));
...
fn load_pe(fd: &mut File, name: String) -> Result<(Project, Machine)> {
        ...
        let (layer, size) = {
            ...
            if size > 0 {
                if offset + size >= bytes.len() {
                    debug!("bad section pointer: {:#x} + {:#x} >= {:#x}",
                           offset,
                           size,
                           bytes.len());
                    (Layer::undefined(0), 0)
    ...
    prog.call_graph.add_vertex(CallTarget::Todo(Rvalue::new_u64(entry),
                                                Some(name.to_string()),
                                                Uuid::new_v4()));
    ...
    for export in pe.exports {
        ...
        prog.call_graph
            .add_vertex(CallTarget::Todo(Rvalue::new_u64(export.rva as u64 + image_base),
                                         Some(export.name),
                                         Uuid::new_v4()));
    }

    for import in pe.imports {
        debug!("adding import: {:?} @ {:#x}",
               &import,
...
pub fn load(path: &Path) -> Result<(Project, Machine)> {
    ...
    match goblin::peek(&mut fd)? {
        Hint::Elf(_) => load_elf(&mut fd, name),
        Hint::PE => load_pe(&mut fd, name),
        // wip
        Hint::Mach => {
        ...
        _ => Err("Tried to load an unknown file".into()),
    }
}
