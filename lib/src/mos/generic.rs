...
pub fn disassembler() -> Rc<Disassembler<Mos>> {
    ...
    new_disassembler!(Mos =>
        // ADC
	[ 0x61, izx ] = unary_izx("adc", nop_r),	// 011 000 01 xxxx xxxx
	[ 0x65, zpg ] = unary_z("adc", nop_r),		// 011 001 01 zzzz zzzz
	[ 0x69, imm ] = unary_i("adc", nop_r),		// 011 010 01 iiii iiii
	[ 0x6d, abs ] = unary_a("adc", nop_r),		// 011 011 01 aaaa aaaa
	[ 0x71, izy ] = unary_izy("adc", nop_r),	// 011 100 01 yyyy yyyy
	[ 0x75, zpg ] = unary_zr("adc", &*X, nop_rr),	// 011 101 01 zzzz zzzz,X
	[ 0x79, abs ] = unary_ar("adc", &*Y, nop_rr),	// 011 110 01 aaaa aaaa,Y
	[ 0x7d, abs ] = unary_ar("adc", &*X, nop_rr),	// 011 111 01 aaaa aaaa,X

	// AND
	[ 0x21, izx ] = unary_izx("and", nop_r),	// 001 000 01 xxxx xxxx
	[ 0x25, zpg ] = unary_z("and", nop_r),		// 001 001 01 zzzz zzzz
        [ 0x29, imm ] = unary_i("and", nop_r),		// 001 010 01 iiii iiii
	[ 0x2d, abs ] = unary_a("and", nop_r),		// 001 011 01 aaaa aaaa
	[ 0x31, izy ] = unary_izy("and", nop_r),	// 001 100 01 yyyy yyyy
	[ 0x35, zpg ] = unary_zr("and", &*X, nop_rr),	// 001 101 01 zzzz zzzz,X
	[ 0x39, abs ] = unary_ar("and", &*Y, nop_rr),	// 001 110 01 aaaa aaaa,X
	[ 0x3d, abs ] = unary_ar("and", &*X, nop_rr),	// 001 111 01 aaaa aaaa,Y

	// ASL
	[ 0x02 ] = nonary("kil!", nop),			// 000 00 010*
	[ 0x0a ] = unary_r("asl", &*A, nop_r),		// 000 01 010  A
	[ 0x12 ] = nonary("kil!", nop),			// 000 10 010*
	[ 0x1a ] = nonary("nop!", nop),	     		// 000 11 010*
	// ASL arg
	[ 0x06, zpg ] = unary_z("asl", nop_r),		// 000 00 110  zzzz zzzz
	[ 0x0e, abs ] = unary_a("asl", nop_r),		// 000 01 110  aaaa aaaa
	[ 0x16, zpg ] = unary_zr("asl", &*X, nop_rr),	// 000 10 110  zzzz zzzz,X
	[ 0x1e, abs ] = unary_ar("asl", &*X, nop_rr),   // 000 11 110  aaaa aaaa,X
	...
	// BIT
	[ 0x24, zpg ] = unary_z("bit", nop_r),			// 0010 0 100 zzzz zzzz
	[ 0x2c, abs ] = unary_a("bit", nop_r),			// 0010 1 100 aaaa aaaa
	...
	// BRK
	[ 0x00 ] = nonary("brk", nop),				// 0000 0000

	// CLx
	[ 0x18 ] = nonary("clc", nop),				// 00 011000
	[ 0x58 ] = nonary("cli", nop),				// 01 011000
	       	   		 				// 10 011000 is tya!!!
	[ 0xd8 ] = nonary("cld", nop),				// 11 011000

	// SEx (no SEV!)
	[ 0x38 ] = nonary("sec", nop),				// 00 111000
	[ 0x78 ] = nonary("sei", nop),				// 01 111000
	[ 0xb8 ] = nonary("clv", nop),				// 10 111000 Odd one out! Should logically be sev, but that doesn't exist.  switched place with tya
	[ 0xf8 ] = nonary("sed", nop),				// 11 111000

	// CMP
	[ 0xc1, izx ] = unary_izx("cmp", nop_r),		// 110 000 01
	[ 0xc5, zpg ] = unary_z("cmp", nop_r),			// 110 001 01
	[ 0xc9, imm ] = unary_i("cmp", nop_r),			// 110 010 01
	[ 0xcd, abs ] = unary_a("cmp", nop_r),			// 110 011 01
	[ 0xd1, izy ] = unary_izy("cmp", nop_r),		// 110 100 01
	[ 0xd5, zpg ] = unary_zr("cmp", &*X, nop_rr),		// 110 101 01
	[ 0xd9, abs ] = unary_ar("cmp", &*Y, nop_rr),		// 110 110 01
	[ 0xdd, abs ] = unary_ar("cmp", &*X, nop_rr),		// 110 111 01

	// CPx
	[ 0xc0, imm ] = unary_i("cpy", nop_r),			// 11 0 0 00 00
	[ 0xc4, zpg ] = unary_z("cpy", nop_r),			// 11 0 0 01 00
	  	      		       				// 11 0 0 10 00 Odd one out 0xc8 iny
	[ 0xcc, abs ] = unary_a("cpy", nop_r),			// 11 0 0 11 00
	[ 0xe0, imm ] = unary_i("cpx", nop_r),			// 11 1 0 00 00
	[ 0xe4, zpg ] = unary_z("cpx", nop_r),			// 11 1 0 01 00
	  	      		       				// 11 1 0 10 00 Odd one out 0xe8 inx
	[ 0xec, abs ] = unary_a("cpx", nop_r),			// 11 1 0 11 00
	...
	// EOR
	[ 0x41, izx ] = unary_izx("eor", nop_r),		// 010 000 01
	[ 0x45, zpg ] = unary_z("eor", nop_r),			// 010 001 01
	[ 0x49, imm ] = unary_i("eor", nop_r),			// 010 010 01
	[ 0x4d, abs ] = unary_a("eor", nop_r),			// 010 011 01
	[ 0x51, izy ] = unary_izy("eor", nop_r),		// 010 100 01
	[ 0x55, zpg ] = unary_zr("eor", &*X, nop_rr),		// 010 101 01
	[ 0x59, abs ] = unary_ar("eor", &*Y, nop_rr),		// 010 110 01
	[ 0x5d, abs ] = unary_ar("eor", &*X, nop_rr),		// 010 111 01
	...
	// LDA
	[ 0xa1, izx ] = unary_izx("lda", nop_r),	// 101 000 01
	[ 0xa5, zpg ] = unary_z("lda", nop_r),		// 101 001 01
	[ 0xa9, imm ] = unary_i("lda", nop_r),		// 101 010 01
	[ 0xad, abs ] = unary_a("lda", nop_r),		// 101 011 01
	[ 0xb1, izy ] = unary_izy("lda", nop_r),	// 101 100 01
	[ 0xb5, zpg ] = unary_zr("lda", &*X, nop_rr),	// 101 101 01
	[ 0xb9, abs ] = unary_ar("lda", &*Y, nop_rr),	// 101 110 01
	[ 0xbd, abs ] = unary_ar("lda", &*X, nop_rr),	// 101 111 01

	// LDX
	[ 0xa2, imm ] = unary_i("ldx", nop_r),		// 101 000 10
	[ 0xa6, zpg ] = unary_z("ldx", nop_r),		// 101 001 10
	  	      		       			// 101 010 10 0xaa is tax
	[ 0xae, abs ] = unary_a("ldx", nop_r),		// 101 011 10
	[ 0xb2 ] = nonary("kil!", nop),			// 101 100 10*
	[ 0xb6, zpg ] = unary_zr("ldx", &*Y, nop_rr),	// 101 101 10
	  	      			     		// 101 110 10 0xba is tsx
	[ 0xbe, abs ] = unary_ar("ldx", &*Y, nop_rr),	// 101 111 10
	...
	// LSR
	[ 0x42 ] = nonary("kil!", nop),			// 010 00 0 10*
	[ 0x4a ] = unary_r("lsr", &*A, nop_r),		// 010 01 0 10
	[ 0x52 ] = nonary("kil!", nop),			// 010 10 0 10*
	[ 0x5a ] = nonary("nop!", nop),	     		// 010 11 0 10
	[ 0x46, zpg ] = unary_z("lsr", nop_r),		// 010 00 1 10 zzzz zzzz
	[ 0x4e, abs ] = unary_a("lsr", nop_r),		// 010 01 1 10 aaaa aaaa
	[ 0x56, zpg ] = unary_zr("lsr", &*X, nop_rr),	// 010 10 1 10 zzzz zzzz
	[ 0x5e, abs ] = unary_ar("lsr", &*X, nop_rr),	// 010 11 1 10 aaaa aaaa

	// NOP
	[ 0xea ] = nonary("nop", nop),

	// ORA
	[ 0x01, izx ] = unary_izx("ora", nop_r),	// 000 000 01
	[ 0x05, zpg ] = unary_z("ora", nop_r),		// 000 001 01
	[ 0x09, imm ] = unary_i("ora", nop_r),		// 000 010 01
	[ 0x0d, abs ] = unary_a("ora", nop_r),		// 000 011 01
	[ 0x11, izy ] = unary_izy("ora", nop_r),	// 000 100 01
	[ 0x15, zpg ] = unary_zr("ora", &*X, nop_rr),	// 000 101 01
	[ 0x19, abs ] = unary_ar("ora", &*Y, nop_rr),	// 000 110 01
	[ 0x1d, abs ] = unary_ar("ora", &*X, nop_rr),	// 000 111 01
	...
	// ROx
	[ 0x22 ] = nonary("kil!", nop),    		// 0 0 1 00 0 10*
	[ 0x2a ] = unary_r("rol", &*A, nop_r),		// 0 0 1 01 0 10
	[ 0x32 ] = nonary("kil!", nop),			// 0 0 1 10 0 10*
	[ 0x3a ] = nonary("nop!", nop),			// 0 0 1 11 0 10*
	[ 0x26, zpg ] = unary_z("rol", nop_r),		// 0 0 1 00 1 10
	[ 0x2e, abs ] = unary_a("rol", nop_r),		// 0 0 1 01 1 10
	[ 0x36, zpg ] = unary_zr("rol", &*X, nop_rr),	// 0 0 1 10 1 10
	[ 0x3e, abs ] = unary_ar("rol", &*X, nop_rr),	// 0 0 1 11 1 10
	[ 0x62 ] = nonary("kil!", nop),    		// 0 1 1 00 0 10*
	[ 0x6a ] = unary_r("ror", &*A, nop_r),		// 0 1 1 01 0 10
	[ 0x72 ] = nonary("kil!", nop),			// 0 1 1 10 0 10*
	[ 0x7a ] = nonary("nop!", nop),			// 0 1 1 11 0 10*
	[ 0x66, zpg ] = unary_z("ror", nop_r),		// 0 1 1 00 1 10
	[ 0x6e, abs ] = unary_a("ror", nop_r),		// 0 1 1 01 1 10
	[ 0x76, zpg ] = unary_zr("ror", &*X, nop_rr),	// 0 1 1 10 1 10
	[ 0x7e, abs ] = unary_ar("ror", &*X, nop_rr),	// 0 1 1 11 1 10

	// RTI
	[ 0x40 ] = nonary_ret("rti", nop),		// 0100 0000

	// RTS
	[ 0x60 ] = nonary_ret("rts", nop),		// 0110 0000

	// SBC
	[ 0xe1, izx ] = unary_izx("sbc", nop_r),	// 111 000 01
	[ 0xe5, zpg ] = unary_z("sbc", nop_r),		// 111 001 01
	[ 0xe9, imm ] = unary_i("sbc", nop_r),		// 111 010 01
	[ 0xed, abs ] = unary_a("sbc", nop_r),		// 111 011 01
	[ 0xf1, izy ] = unary_izy("sbc", nop_r),	// 111 100 01
	[ 0xf5, zpg ] = unary_zr("sbc", &*X, nop_rr),	// 111 101 01
	[ 0xf9, abs ] = unary_ar("sbc", &*Y, nop_rr),	// 111 110 01
	[ 0xfd, abs ] = unary_ar("sbc", &*X, nop_rr),	// 111 111 01

	// STA
	[ 0x81, izx ] = unary_izx("sta", nop_r),	// 100 000 01
	[ 0x85, zpg ] = unary_z("sta", nop_r),		// 100 001 01
	[ 0x89, imm ] = unary_i("nop!", nop_r),		// 100 010 01* illegal nop imm
	[ 0x8d, abs ] = unary_a("sta", nop_r),		// 100 011 01
	[ 0x91, izy ] = unary_izy("sta", nop_r),	// 100 100 01
	[ 0x95, zpg ] = unary_zr("sta", &*X, nop_rr),	// 100 101 01
	[ 0x99, abs ] = unary_ar("sta", &*Y, nop_rr),	// 100 110 01
	[ 0x9d, abs ] = unary_ar("sta", &*X, nop_rr),	// 100 111 01

	// STx
	[ 0x86, zpg ] = unary_z("stx", nop_r),		// 100 00 1 1 0
	[ 0x96, zpg ] = unary_zr("stx", &*Y, nop_rr),	// 100 10 1 1 0
	[ 0x8e, abs ] = unary_a("stx", nop_r),		// 100 01 1 1 0
	[ 0x9e, abs ] = unary_ar("shx!", &*Y, nop_rr),  // 100 11 1 1 0* ill shx absy
	[ 0x84, zpg ] = unary_z("sty", nop_r),		// 100 00 1 0 0
	[ 0x8c, abs ] = unary_a("sty", nop_r),		// 100 01 1 0 0
	[ 0x94, zpg ] = unary_zr("sty", &*X, nop_rr),	// 100 10 1 0 0
	[ 0x9c, abs ] = unary_ar("shy!", &*X, nop_rr),  // 100 11 1 0 0* ill shx absy

	// Txy - no pattern :-/
	[ 0xaa ] = nonary("tax", nop),			// 10 1 0 10 1 0
	[ 0xa8 ] = nonary("tay", nop),			// 10 1 0 10 0 0
	       	   		 			// 10 1 1 10 0 0  0xb8 clv !!!
	[ 0xba ] = nonary("tsx", nop),			// 10 1 1 10 1 0
	[ 0x8a ] = nonary("txa", nop),			// 10 0 0 10 1 0
	       	   		 			// 10 0 0 10 0 0  0x88 dey
	[ 0x9a ] = nonary("txs", nop),			// 10 0 1 10 1 0
	[ 0x98 ] = nonary("tya", nop),			// 10 0 1 10 0 0
	...
	[ 0xd2 ] = nonary("kil!", nop),			// 1101 0010*
	[ 0xe2, imm ] = unary_i("nop!", nop_r),		// 1100 0010* iiii iiii
	[ 0xf2 ] = nonary("kil!", nop),			// 1111 0010*
