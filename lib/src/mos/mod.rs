...
use {Lvalue, Rvalue, Guard, State, Statement, Architecture, LayerIter, Region, Result, Disassembler, Match};
use std::sync::Arc;
use std::borrow::Cow;
...
impl Architecture for Mos {
    ...
    fn prepare(reg: &Region, _: &Self::Configuration) -> Result<Vec<(&'static str, u64, &'static str)>> {
        let mut i = reg.iter();
        let iv = vec![("NMI", 0xfffa, "NMI vector"), ("RESET", 0xfffc, "Reset routine"), ("IRQ/BRK", 0xfffe, "Interrupt routine")];
        let mut ret = vec![];
...
pub fn nonary(opcode: &'static str, sem: fn(&mut Variant) -> Result<Vec<Statement>>) -> Box<Fn(&mut State<Mos>) -> bool> {
    Box::new(move |st: &mut State<Mos>| -> bool {
        ...
        st.mnemonic_dynargs(len,
                            &opcode,
                            "",
                            &|c| -> Result<(Vec<Rvalue>, Vec<Statement>)> { Ok((vec![], try!(sem(c)))) });
    ...
    Box::new(move |st: &mut State<Mos>| -> bool {
        ...
        if let Some(arg) = _arg {
            st.mnemonic_dynargs(len,
                                &opcode,
                                "#{u}",
                                &|c| -> Result<(Vec<Rvalue>, Vec<Statement>)> { Ok((vec![arg.clone()], try!(sem(c, arg.clone())))) });
            st.jump(Rvalue::new_u16(next), Guard::always());
            true
        } else {
            false
