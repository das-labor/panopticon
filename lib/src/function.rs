...
use graph_algos::{AdjacencyList, GraphTrait, MutableGraphTrait, VertexListGraphTrait, EdgeListGraphTrait, BidirectionalGraphTrait};
use graph_algos::adjacency_list::{AdjacencyListVertexDescriptor, AdjacencyListEdgeDescriptor};
use graph_algos::search::{TraversalOrder, TreeIterator};
use uuid::Uuid;

use {BasicBlock, Guard, Region, Disassembler, Architecture, LayerIter, Rvalue, Mnemonic, Statement, Operation};
...
impl Function {
            ...
            match g.vertex_label(v) {
                Some(&ControlFlowTarget::Resolved(ref bb)) => {
                    ...
                    for mne in &bb.mnemonics {
                        mnemonics.entry(mne.area.start)
                            .or_insert(Vec::new())
                            .push(MnemonicOrError::Mnemonic(mne.clone()));

                        if let Some(prev) = prev_mne {
                            by_source.entry(prev)
                                .or_insert(Vec::new())
                                .push((Rvalue::new_u64(mne.area.start), Guard::always()));
                            by_destination.entry(mne.area.start)
                                .or_insert(Vec::new())
        ...
        for e in g.edges() {
            ...
            match (src, tgt) {
                ...
                (Some(&ControlFlowTarget::Resolved(ref src_bb)), Some(&ControlFlowTarget::Resolved(ref tgt_bb))) => {
                    ...
                    by_source.entry(last)
                        .or_insert(Vec::new())
                        .push((Rvalue::new_u64(tgt_bb.area.start), gu.clone()));
                    by_destination.entry(tgt_bb.area.start)
                        .or_insert(Vec::new())
    ...
    fn assemble_cflow_graph(mut mnemonics: BTreeMap<u64, Vec<MnemonicOrError>>, by_source: HashMap<u64, Vec<(Rvalue, Guard)>>, by_destination: HashMap<u64, Vec<(Rvalue, Guard)>>, start: u64) -> ControlFlowGraph {
        ...
        for (_, mnes) in mnemonics.iter_mut() {
            if !bblock.is_empty() && !mnes.is_empty() {
                if let Some(&MnemonicOrError::Mnemonic(ref mne)) = mnes.first() {
                    ...
                    new_bb |= by_source.get(&last_mne.area.start)
                        .unwrap_or(&Vec::new())
                        .iter()
                        .any(|&(ref opt_dest, _)| {
                            if let &Rvalue::Constant { value, .. } = opt_dest {
                    ...
                    new_bb |= by_destination.get(&mne.area.start)
                        .unwrap_or(&Vec::new())
                        .iter()
                        .any(|&(ref opt_src, _)| {
                            if let &Rvalue::Constant { value, .. } = opt_src {
        ...
        for (src_off, tgts) in by_source.iter() {
            for &(ref tgt, ref gu) in tgts {
                ...
                match (from_bb, to_bb) {
                    ...
                    _ => {
                        trace!("jump from 0x{:x} to {} doesn't hit any blocks",
                               src_off,
                               tgt)
                    }
                }
              ...
              A::Configuration: Debug
    {
        ...
        let (mut mnemonics, mut by_source, mut by_destination) = cont.map_or((BTreeMap::new(), HashMap::new(), HashMap::new()),
                    ...
                    match mnes.first() {
                        Some(&MnemonicOrError::Mnemonic(ref mne)) => {
                            if mne.area.start < addr && mne.area.end > addr {
                                mnemonics.entry(addr)
                                    .or_insert(Vec::new())
                                    .push(MnemonicOrError::Error(addr, "Jump inside instruction".into()));
                                continue;
            ...
            match maybe_match {
                Ok(match_st) => {
                    if match_st.mnemonics.is_empty() {
                        mnemonics.entry(addr)
                            .or_insert(Vec::new())
                            .push(MnemonicOrError::Error(addr, "Unrecognized instruction".into()));
                    } else {
                    ...
                    for (origin, tgt, gu) in match_st.jumps {
                        ...
                        match tgt {
                            Rvalue::Constant { value: ref c, .. } => {
                                by_source.entry(origin)
                                    .or_insert(Vec::new())
                                    .push((tgt.clone(), gu.clone()));
                                by_destination.entry(*c)
                                    .or_insert(Vec::new())
                                    .push((Rvalue::new_u64(origin), gu.clone()));
                                todo.insert(*c);
                            }
                            _ => {
                                by_source.entry(origin)
    ...
    pub fn postorder(&self) -> Vec<ControlFlowRef> {
        ...
        TreeIterator::new(self.entry_point.unwrap(),
...
mod tests {
    ...
    use {Guard, Mnemonic, Bound, BasicBlock, Region, Match, Rvalue, OpaqueLayer, LayerIter, Result, State, Architecture, Disassembler};
    ...
    fn index_resolved() {
        ...
        let bb0 = BasicBlock::from_vec(vec![Mnemonic::dummy(0..1), Mnemonic::dummy(1..2), Mnemonic::dummy(2..5), Mnemonic::dummy(5..6)]);
        let bb1 = BasicBlock::from_vec(vec![Mnemonic::dummy(10..11), Mnemonic::dummy(11..12), Mnemonic::dummy(12..15), Mnemonic::dummy(15..16)]);
        ...
        for e in cfg_re.edges() {
            if let Some(&ControlFlowTarget::Resolved(ref from)) = cfg_re.vertex_label(cfg_re.source(e)) {
                if let Some(&ControlFlowTarget::Resolved(ref to)) = cfg_re.vertex_label(cfg_re.target(e)) {
                    assert!((from.area.start == 0 && to.area.start == 10) || (from.area.start == 10 && to.area.start == 10) || (from.area.start == 10 && to.area.start == 6) || (from.area.start == 6 && to.area.start == 0));
                } else {
                    unreachable!();
                }
            } else {
    ...
    fn index_unresolved() {
        ...
        let vx4 = cfg.add_vertex(ControlFlowTarget::Unresolved(Rvalue::Variable {
            name: Cow::Borrowed("a"),
            size: 8,
            offset: 0,
            subscript: None,
