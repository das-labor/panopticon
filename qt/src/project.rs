...
use panopticon::{Project, Function, Program, CallTarget, ControlFlowTarget, Rvalue, Result, ssa_convertion, Lvalue, Architecture, OpaqueLayer, Layer, Region, Bound, World, approximate, Kset};
...
use qmlrs::Variant;
use graph_algos::{VertexListGraphTrait, MutableGraphTrait, GraphTrait};
use controller::{DISCOVERED_FUNCTION, STARTED_FUNCTION, FINISHED_FUNCTION, Controller, return_json};
use uuid::Uuid;
            ...
            if let &Variant::I64(entry) = _entry {
                if let Some(nam) = p.file_name().and_then(|x| x.to_str()).or(p.to_str()) {
                    if let Ok(b) = OpaqueLayer::open(p) {
                        ...
                        reg.cover(Bound::new(base as u64, base as u64 + b.iter().len()),
                                  Layer::Opaque(b));
                        ...
                        if let &Variant::String(ref tgt_s) = _tgt {
                            ...
                            if let Ok(ref iv) = iv {
                                ...
                                if entry >= 0 {
                                    let uu = Uuid::new_v4();
                                    prog.call_graph.add_vertex(CallTarget::Todo(Rvalue::new_u64(entry as u64),
                                                                                Some("Entry point".to_string()),
                                                                                uu));
                                    proj.comments.insert((nam.to_string(), entry as u64),
                                                         "User supplied entry point".to_string());
                                } else {
                                    for &(name, off, cmnt) in iv.iter() {
                                        let uu = Uuid::new_v4();
                                        prog.call_graph
                                            .add_vertex(CallTarget::Todo(Rvalue::new_u64(off), Some(name.to_string()), uu));
                                        proj.comments
                                            .insert((nam.to_string(), off), cmnt.to_string());
                                    }
                                }
...
pub fn snapshot_project(_path: &Variant) -> Variant {
    Variant::String(if let &Variant::String(ref s) = _path {
        return_json(Controller::set_backing(&Path::new(s)).and_then(|_| Controller::sync()))
    } else {
        return_json::<()>(Err("1st argument is not a string".into()))
    })
}
...
/// Starts disassembly
pub fn spawn_disassembler<A: 'static + Architecture + Debug>(_cfg: A::Configuration)
    where A::Configuration: Debug + Sync,
          A::Token: Sync + Send
{
    use std::sync::Mutex;

    thread::spawn(move || -> Result<()> {
        let maybe_prog_uuid = try!(Controller::read(|proj| proj.code.first().map(|x| x.uuid)));
                    ...
                    Some((Rvalue::Constant { value: tgt, .. }, maybe_name, uuid)) => {
                        ...
                        let th = thread::spawn(move || -> Result<Vec<Uuid>> {
                            ...
                            while !fixpoint {
                                ...
                                let vxs = {
                                    func.cflow_graph.vertices().collect::<Vec<_>>()
                                };
        ...
        let uuids = try!(Controller::read(|proj| {
            proj.code
                ...
                .flat_map(|p| {
                    p.call_graph
                        .vertices()
                        .filter_map(move |vx| p.call_graph.vertex_label(vx).map(|x| x.uuid()))
                })
                .collect::<Vec<_>>()
        }));
