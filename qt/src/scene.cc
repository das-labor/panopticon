...
std::pair<QMultiMap<Node *,Arrow *>::iterator,QMultiMap<Node *,Arrow *>::iterator> Scene::out_edges(Node *n) 
{
	return std::make_pair(m_incidence.lowerBound(n),m_incidence.upperBound(n));
}

QRectF Scene::graphLayout(void)
{
	if(nodes().empty())
		return QRectF();
...
void Scene::connect(Node *a, Node *b)
{
