...
pub fn linear_layout_start(vertices: &Vec<usize>,
                           edges: &Vec<(usize,usize)>,
...
pub fn linear_layout_order(mut layout: LinearLayout) -> Result<LinearLayout,Error> {
    match layout {
        ...
        LinearLayout::Ordering{ iterations_left: ref mut iters, ref mut order, ref rank, ref graph, ref bipartite, ref mut xings,.. } => {
            optimize_ordering_once(*iters,bipartite,xings,order,rank,graph);
            if *xings == 0 { *iters = 0; } else { *iters -= 1; }
        }
...
pub fn linear_layout_placement(vertices: &Vec<usize>,
                               edges: &Vec<(usize,usize)>,
                               layout: &LinearLayout,
                               dims: &HashMap<usize,(f32,f32)>,
                               node_spacing: f32, rank_spacing: f32,
    ...
    if let &LinearLayout::Ordering{ ref order, ref rank, ref graph, ref rev, virt_start, ref revd_edge_labels,.. } = layout {
        let mut graph = graph.clone();
        let rank_sep = rank_spacing + block_spacing + entry_spacing;
        let dims = HashMap::from_iter(graph.vertices().map(|vx| {
            (vx,graph.vertex_label(vx).and_then(|x| dims.get(x).cloned()).unwrap_or((0.,0.)))
        }));
        ...
        for n in vertices.iter() {
            ...
            let sort_by_order = |a: &(AdjacencyListEdgeDescriptor,AdjacencyListVertexDescriptor),
            b: &(AdjacencyListEdgeDescriptor,AdjacencyListVertexDescriptor)| -> Ordering {
        ...
        let x_pos = compute_x_coordinates(order,rank,&graph,&dims,&x_off,&|_| node_spacing,virt_start);

        let rank_offsets = order.iter()
            .map(|r| {
                r.iter().fold(0.,|acc,vx| {
                    let h = dims.get(vx).map(|x| {
                        assert!(x.1 >= 0.0);
                        x.1
                    }).unwrap_or(0.);
                    partial_max(h,acc)
        ...
        for lb in revd_edge_labels.iter() {
            if let Some(&(start,end)) = end_points.get(lb) {
                ...
                let s = order[start_r].
                    ...
                    fold(0.0,|acc,x| if acc < dims.get(x).unwrap_or(&(0.,0.)).1 { dims.get(x).unwrap_or(&(0.,0.)).1 } else { acc });
...
pub fn mark_type1_conflicts(virt_start: usize,
                            order: &Vec<Vec<AdjacencyListVertexDescriptor>>,
                            rank: &HashMap<AdjacencyListVertexDescriptor,isize>,
                            graph: &AdjacencyList<usize,usize>) -> Vec<(AdjacencyListVertexDescriptor,AdjacencyListVertexDescriptor)> {
    ...
    if v == w {
        ...
        if let Some(e) = graph.in_edges(w).next() {
            ...
            if placed.contains(&r) {
                threshold = x[&r] + inner_shift[&p] + port_offset.get(&e).map(|x| x.0).unwrap_or(0.)
                                  - inner_shift[&q] - port_offset.get(&e).map(|x| x.1).unwrap_or(0.);
            } else {
                queue.push((w,e));
            }
        }
    ...
    if !x.contains_key(&v) {
        ...
        loop {
            ...
            if (left_to_right && *word.first().unwrap() != w) ||
                ...
                place_block(u,order,rank,sink,shift,x,port_offset,align,root,dims,inner_shift,placed,queue,graph,node_spacing,left_to_right);

                threshold = calculate_threshold(v, w, threshold, left_to_right,queue,placed,x,port_offset,align,root,inner_shift,graph);
                ...
                if sink[&v] != sink[&u] {
                    ...
                    if !left_to_right {
                        shift.insert(sinku.clone(),partial_min(prev,x[&v] + inner_shift[&w] - x[&u] - inner_shift[&n] - dims.get(&n).unwrap_or(&(0.,0.)).0 - spacing));
                    } else {
                        shift.insert(sinku.clone(),partial_max(prev,x[&v] + inner_shift[&w] - (x[&u] + inner_shift[&n]) + dims.get(&n).unwrap_or(&(0.,0.)).0 + spacing));
                    }
                } else {
                    let sb = if !left_to_right {
                        partial_max(threshold,x[&u] + inner_shift[&n] + dims.get(&n).unwrap_or(&(0.,0.)).0 - inner_shift[&w] + spacing)
                    } else {
                        partial_min(threshold,x[&u] + inner_shift[&n] - dims.get(&n).unwrap_or(&(0.,0.)).0 - inner_shift[&w] - spacing)
                    };
...
fn post_process(x: &mut HashMap<AdjacencyListVertexDescriptor,f32>,
                port_offset: &HashMap<AdjacencyListEdgeDescriptor,(f32,f32)>,
                align: &HashMap<AdjacencyListVertexDescriptor,AdjacencyListVertexDescriptor>,
                root: &HashMap<AdjacencyListVertexDescriptor,AdjacencyListVertexDescriptor>,
                inner_shift: &HashMap<AdjacencyListVertexDescriptor,f32>,
    ...
    for &(vx,e) in queue {
        ...
        let rp = root[&p];
        let rq = root[&q];
        let t = x[&rp] + inner_shift[&p] + port_offset.get(&e).map(|x| x.0).unwrap_or(0.)
              - x[&rq] + inner_shift[&q] + port_offset.get(&e).map(|x| x.1).unwrap_or(0.);
...
fn horizontal_compaction(order: &Vec<Vec<AdjacencyListVertexDescriptor>>,
                         ...
                         node_spacing: &Fn(usize) -> f32,
                         up_to_down: bool,
                         left_to_right: bool) -> HashMap<AdjacencyListVertexDescriptor,f32> {
    ...
    while vi != vto {
        ...
        while hi != hto {
            ...
            if root[&v] == v {
                place_block(v,order,rank,&mut sink,&mut shift,&mut x,port_offset,&align,&root,
                            dims,inner_shift,&mut placed,&mut queue,graph,node_spacing,left_to_right);
            }
    ...
    post_process(&mut x,port_offset,align,root,inner_shift,&queue,graph,left_to_right);

    x
}

pub fn inner_shift(root: &HashMap<AdjacencyListVertexDescriptor,AdjacencyListVertexDescriptor>,
                   dims: &HashMap<AdjacencyListVertexDescriptor,(f32,f32)>,
                   port_offset: &HashMap<AdjacencyListEdgeDescriptor,(f32,f32)>,
                   graph: &AdjacencyList<usize,usize>)
    -> (HashMap<AdjacencyListVertexDescriptor,f32>,HashMap<AdjacencyListVertexDescriptor,f32>)
{
    ...
    for e in graph.edges() {
        ...
        if r == root[&q] {
            ...
            let s = inner_shift[&p] + port_offset.get(&e).map(|x| x.0).unwrap_or(0.)
...
pub fn compute_x_coordinates(order: &Vec<Vec<AdjacencyListVertexDescriptor>>,
    ...
    let layouts = (0..4).map(|k| {
        ...
        let position = horizontal_compaction(&order,&rank,&graph,&align,&root,&dims,&port_offset,&inner_shift,node_spacing,up_to_down,left_to_right);
    ...
    let min_max = layouts.iter().enumerate().map(|(_,&(_,_,ref pos,ref inner_shift,_))| {
        ...
        let max = pos.iter().max_by(|&(vx,x),&(wx,y)| {
            (dims.get(vx).unwrap_or(&(0.,0.)).0 + x + inner_shift[vx]).partial_cmp(
            &(dims.get(wx).unwrap_or(&(0.,0.)).0 + y + inner_shift[wx])).unwrap_or(Ordering::Equal)
        }).unwrap();
    ...
    let final_position = HashMap::from_iter(graph.vertices().map(|vx| {
        let mut candidate_pos = layouts.iter().zip(shift.iter()).map(|(&(_,_,ref position,ref inner_shift,_),shift)| {
            position[&vx] + inner_shift[&vx] + shift
        }).collect::<Vec<_>>();
