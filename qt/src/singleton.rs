...
impl Panopticon {
    ...
    pub fn open_program(&mut self,path: String) -> Result<()> {
        ...
        if let Ok(proj) = Project::open(&Path::new(&path)) {
            if !proj.code.is_empty() {
                ...
                glue::send_current_session(CString::new(path.as_bytes())?)
            } else {
                Ok(())
            }
            ...
            if let Some(prog) = maybe_prog {
                ...
                glue::send_current_session(CString::new(dir.as_bytes())?)
            } else {
    ...
    pub fn undo(&mut self) -> Result<()> {
        ...
        glue::send_undo_redo_update(top - 1 != 0,true)
    }
    ...
    pub fn redo(&mut self) -> Result<()> {
        ...
        glue::send_undo_redo_update(true,top + 1 < len)
    }
    ...
    pub fn update_control_flow_nodes(&mut self,uuid: &Uuid, addrs: Option<&[u64]>) -> Result<()> {
        ...
        let bbls: Vec<_> = self
            ...
            .into_iter()
            .filter_map(|bbl| {
                if bbl.4.is_empty() || ids.is_empty() || ids.iter().any(|&x| x == bbl.0 as i32) {
                    Some(bbl)
                } else {
                    None
                }
