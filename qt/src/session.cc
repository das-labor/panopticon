...
: QObject(p)
{
	...
	po::region_loc base_as(new po::region("base",1280));
	po::region_loc xor_as(new po::region("xor",640));
	po::region_loc add_as(new po::region("add",270));
...
//	m_graph.insert_node(aes_as);
	...
	_regions.insert_edge(po::bound(100,740),xor_desc,base_desc);
	_regions.insert_edge(po::bound(800,1070),add_desc,base_desc);
}
