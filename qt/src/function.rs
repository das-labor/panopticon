...
use panopticon;
use panopticon::{Rvalue, Lvalue, Function, ControlFlowTarget, CallTarget, MnemonicFormatToken, Error, Result, Kset};
...
use qmlrs::Variant;
use graph_algos::{VertexListGraphTrait, GraphTrait, IncidenceGraphTrait, EdgeListGraphTrait};
use uuid::Uuid;
use rustc_serialize::{json, Encodable, Encoder};
use byteorder::{BigEndian, ReadBytesExt};
use controller::{LAYOUTED_FUNCTION, CHANGED_FUNCTION, return_json, Controller};
            ...
            let ret = Controller::read(|proj| {
                if let Some((vx, prog)) = proj.find_call_target_by_uuid(&tgt_uuid) {
                    ...
                    match prog.call_graph.vertex_label(vx) {
                        ...
                        Some(&CallTarget::Concrete(Function { ref uuid,
                                                              ref name,
                                                              entry_point: None,
                                                              .. })) => {
                            return_json(Ok(Metainfo {
                                kind: "function",
                        ...
                        None => return_json::<()>(Err("Internal error".into())),
                    }
                } else {
                ...
                if let Some((vx, prog)) = proj.find_call_target_by_uuid(&tgt_uuid) {
                    if let Some(&CallTarget::Concrete(ref fun)) = prog.call_graph.vertex_label(vx) {
                        ...
                        let nodes = cfg.vertices()
                            .filter_map(|x| cfg.vertex_label(x).map(|x| to_ident(x)))
                            .collect();
                        ...
                        let targets = cfg.vertices().filter_map(|x| {
                            let lb = cfg.vertex_label(x);
                            match lb {
                                Some(&ControlFlowTarget::Unresolved(ref rv)) => Some((to_ident(lb.unwrap()), format!("{}", rv))),
                                _ => None,
                            }
                        });
                        let errors = cfg.vertices().filter_map(|x| {
                            let lb = cfg.vertex_label(x);
                            match lb {
                                Some(&ControlFlowTarget::Failed(_, ref msg)) => Some((to_ident(lb.unwrap()), format!("{}", msg))),
                                _ => None,
                            }
                        });
    ...
    Variant::String(if let &Variant::String(ref uuid_str) = arg {
        if let Some(tgt_uuid) = Uuid::parse_str(uuid_str).ok() {
            let ret = Controller::read(|proj| {
                if let Some((vx, prog)) = proj.find_call_target_by_uuid(&tgt_uuid) {
                    if let Some(&CallTarget::Concrete(ref fun)) = prog.call_graph.vertex_label(vx) {
                        return_json(panopticon::approximate::<Kset>(&fun).and_then(|x| {
                            Ok(x.iter()
                                .filter_map(|(k, v)| {
    ...
    let ret = fs::File::open(&path)
        .and_then(|mut fd| {
            ...
            if meta.is_dir() {
                ...
                match peek.unwrap() {
                    Hint::Elf(data) => {
                        let endianness = if data.is_lsb {
                            "LittleEndian"
                        } else {
                            "BigEndian"
                        };
                        let class = if let Some(is_64) = data.is_64 {
...
fn to_ident(t: &ControlFlowTarget) -> String {
    match t {
        &ControlFlowTarget::Resolved(ref bb) => format!("bb{}", bb.area.start),
        &ControlFlowTarget::Unresolved(ref c) => {
            let ref mut h = SipHasher::new();
            c.hash::<SipHasher>(h);
            format!("c{}", h.finish())
        }
        &ControlFlowTarget::Failed(ref pos, _) => format!("err{}", pos),
    }
}
        ...
        if let Some(uuid) = Uuid::parse_str(st).ok() {
            let ret = Controller::read(|proj| {
                if let Some(func) = proj.find_function_by_uuid(&uuid) {
                    ...
                    let edges = func.cflow_graph
                        .edges()
                        .map(|e| {
                            let f = vertices.iter()
                                .position(|&x| x == func.cflow_graph.source(e))
                                .unwrap();
                            let t = vertices.iter()
                                .position(|&x| x == func.cflow_graph.target(e))
                                .unwrap();
                            (f, t)
                        })
                        .collect::<Vec<_>>();
                    let mut dims_transformed = HashMap::<usize, (f32, f32)>::new();
                    let mut unseen_verts = HashSet::<_>::from_iter(vertices.iter()
                        .map(|v| to_ident(func.cflow_graph.vertex_label(*v).unwrap())));

                    for (k, v) in dims.iter() {
                        let _k = vertices.iter()
                            .position(|&x| {
                                let a = to_ident(func.cflow_graph.vertex_label(x).unwrap());
                                a == *k
                            })
                            .unwrap();
                        unseen_verts.remove(&to_ident(func.cflow_graph
                            .vertex_label(vertices[_k])
                            .unwrap()));
                        dims_transformed.insert(_k, (v.width as f32, v.height as f32));
                    }
                    let maybe_entry = func.entry_point
                        .map(|k| vertices.iter().position(|&x| x == k).unwrap());
                    let idents = vertices.iter()
                        .map(|x| to_ident(func.cflow_graph.vertex_label(*x).unwrap()))
                        .collect::<Vec<_>>();
                                        ...
                                        t.modified().ok().and_then(|t| {
                                            t.elapsed().ok().and_then(|x| {
                                                chrono::Duration::from_std(x)
                                                    .ok()
                                                    .and_then(|x| chrono::Duration::zero().checked_sub(&x))
                                            })
                                        })
                                    });
