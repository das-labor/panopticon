...
impl<'a, V, E, G: 'a + Graph<'a,V,E> + IncidenceGraph<'a,V,E> + VertexListGraph<'a,V,E>> TreeIterator<'a,V,E,G> {
    pub fn new(start: G::Vertex, order: TraversalOrder, g: &'a G) -> Self {
        TreeIterator{
            order: order,
            stack: if order == TraversalOrder::Postorder { vec![start] } else { vec![] },
            seen: HashSet::<_>::new(),
            position: start,
            graph: g
        }
...
pub fn is_connected<'a, V, E, G: 'a + Graph<'a,V,E> + IncidenceGraph<'a,V,E> + BidirectionalGraph<'a,V,E> + VertexListGraph<'a,V,E>>(graph: &'a G) -> bool{
    ...
    if let Some(s) = graph.vertices().next() {
        ...
        while !stack.is_empty() {
            ...
            let ed = graph
                .out_edges(vx)
                .map(|out| graph.target(out))
                .chain(graph.in_edges(vx).map(|_in| graph.source(_in)))
                .collect::<Vec<_>>();
...
pub fn depth_first_visit<'a, V, E, G: 'a + Graph<'a,V,E> + IncidenceGraph<'a,V,E> + VertexListGraph<'a,V,E>>(vertex_visitor: &mut FnMut(&G::Vertex,VertexEvent),
...
graph: &'a G){
    ...
    fn visit<'a,V,E,G: 'a + Graph<'a,V,E> + IncidenceGraph<'a,V,E> + VertexListGraph<'a,V,E>>(vx: &G::Vertex,
...
graph: &'a G){
        ...
        for e in graph.out_edges(vx.clone()) {
            ...
            match color[&wx] {
                ...
                VertexColor::Black => {
                    edge_visitor(&e, EdgeKind::ForwardOrCross)
                }
            }
        }
...
mod tests {
    ...
    fn dfs_visit() {
        ...
        let mut vertices =
            HashMap::<AdjacencyListVertexDescriptor, bool>::new();

        depth_first_visit(
            &mut |vx: &AdjacencyListVertexDescriptor, ev| {
                match ev {
                    VertexEvent::Discovered => {
                        assert!(vertices.insert(vx.clone(), false).is_none())
                    }
                    VertexEvent::Finished => {
                        assert!(vertices.insert(vx.clone(), true) == Some(false))
                    }
                }
            },
            &mut |&ed, ev| if ed == e12 {
    ...
    fn preorder() {
        ...
        let preorder =
            iter.map(|vx| tree.vertex_label(vx).unwrap().to_string())
                .collect::<Vec<_>>();
        let expect = vec!["f", "b", "a", "d", "c", "e", "g", "i", "h"]
            .iter()
            .map(|x| x.to_string())
            .collect::<Vec<_>>();
    ...
    fn postorder() {
        ...
        let preorder =
            iter.map(|vx| tree.vertex_label(vx).unwrap().to_string())
                .collect::<Vec<_>>();
        let expect = vec!["a", "c", "e", "d", "b", "h", "i", "g", "f"]
            .iter()
            .map(|x| x.to_string())
            .collect::<Vec<_>>();
