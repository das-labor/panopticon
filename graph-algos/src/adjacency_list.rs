...
pub struct AdjacencyList<N, E> {
    ...
    out_edges:
        HashMap<AdjacencyListVertexDescriptor, Vec<AdjacencyListEdgeDescriptor>>,
    in_edges:
        HashMap<AdjacencyListVertexDescriptor, Vec<AdjacencyListEdgeDescriptor>>,
    edges: HashMap<AdjacencyListEdgeDescriptor,
...
impl<'a, V, E> IncidenceGraph<'a, V, E> for AdjacencyList<V, E> {
    type Incidence = std::iter::Map<std::slice::Iter<'a, Self::Edge>,
                   fn(&Self::Edge) -> Self::Edge>;
...
impl<'a, V, E> AdjacencyGraph<'a, V, E> for AdjacencyList<V, E> {
    ...
    fn adjacent_vertices(&self, v: Self::Vertex) -> Self::Adjacency {
        let i = self.out_edges.get(&v).unwrap().iter().map(
            |&x| {
                return self.target(x);
            }
        );
        let o =
            self.in_edges.get(&v).unwrap().iter().map(
                |&x| return self.source(x),
            );
        let mut raw = i.chain(o)
            .collect::<Vec<AdjacencyListVertexDescriptor>>();
...
impl<'a, V, E: 'a> EdgeListGraph<'a, V, E> for AdjacencyList<V, E> {
    type Edges = std::iter::Map<std::collections::hash_map::Keys<'a,
                                                    Self::Edge,
                                                    E>,
                   fn(&Self::Edge) -> Self::Edge>;
...
impl<'a, V, E> MutableGraph<'a, V, E> for AdjacencyList<V, E> {
    ...
    fn add_edge(
        ...
        if self.vertex_labels.contains_key(&from) &&
           self.vertex_labels.contains_key(&to) {
    ...
    fn remove_edge(&mut self, e: Self::Edge) -> Option<E> {
        ...
        if ret.is_some() {
            ...
            let rm_adj =
                |cont: Option<&mut Vec<AdjacencyListEdgeDescriptor>>,
                 e|
                 -> bool {
                    match cont {
                        None => return false,
    ...
    fn decode<D: Decoder>(d: &mut D) -> Result<Self, D::Error> {
        d.read_struct(
            "AdjacencyList", 3, |d| {
                ...
                let conn: HashMap<usize, (usize, usize)> =
                    try!(
                        d.read_struct_field(
                            "conn",
                            2,
                ...
                let mut vertex_labels =
                    HashMap::<AdjacencyListVertexDescriptor, V>::new();
                let mut edge_labels = HashMap::<AdjacencyListEdgeDescriptor,
                                                E>::new();
                let mut out_edges =
                    HashMap::<AdjacencyListVertexDescriptor,
                              Vec<AdjacencyListEdgeDescriptor>>::new();
                let mut in_edges =
                    HashMap::<AdjacencyListVertexDescriptor,
                              Vec<AdjacencyListEdgeDescriptor>>::new();
                let mut edges =
                    HashMap::<AdjacencyListEdgeDescriptor,
                              (AdjacencyListVertexDescriptor,
                ...
                for c in conn {
                    edges.insert(
                        ...
                        (AdjacencyListVertexDescriptor((c.1).0),
                         AdjacencyListVertexDescriptor((c.1).1)),
                    );
                    out_edges
                        .get_mut(&AdjacencyListVertexDescriptor((c.1).0))
                        .unwrap()
                        .push(AdjacencyListEdgeDescriptor(c.0));
                    in_edges
                        .get_mut(&AdjacencyListVertexDescriptor((c.1).1))
                        .unwrap()
                        .push(AdjacencyListEdgeDescriptor(c.0));
                }
...
mod test {
    ...
    fn test_out_iterator() {
        ...
        assert!(
            e12.is_some() && e23.is_some() && e21.is_some() && e14.is_some()
        );
    ...
    fn decodable() {
        ...
        let g2: AdjacencyList<isize, String> = json::decode(&e.unwrap())
            .unwrap();
