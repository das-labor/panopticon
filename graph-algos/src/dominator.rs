...
pub fn dominators<'a, V, E, G: 'a + Graph<'a,V,E> + BidirectionalGraph<'a,V,E> + VertexListGraph<'a,V,E>>(start: G::Vertex, graph: &'a G) -> HashMap<G::Vertex,Vec<G::Vertex>>{
    let vertex_idx = HashMap::<G::Vertex, usize>::from_iter(
        graph.vertices().enumerate().map(|(a, b)| (b, a)),
    );
    let vertex_ridx =
        HashMap::<usize, G::Vertex>::from_iter(graph.vertices().enumerate());
...
pub fn dominance_frontiers<'a, V, E, G: 'a + Graph<'a,V,E> + BidirectionalGraph<'a,V,E> + VertexListGraph<'a,V,E>>(idom: &HashMap<G::Vertex,G::Vertex>, graph: &'a G) -> HashMap<G::Vertex,Vec<G::Vertex>>{
    let mut ret = HashMap::<G::Vertex, Vec<G::Vertex>>::from_iter(
        graph.vertices().map(|v| (v, vec![])),
    );
...
pub fn immediate_dominator<'a, V, E, G: 'a + Graph<'a,V,E> + BidirectionalGraph<'a,V,E> + VertexListGraph<'a,V,E>>(start: G::Vertex, graph: &'a G) -> HashMap<G::Vertex,G::Vertex>{
    let postorder = TreeIterator::new(start, TraversalOrder::Postorder, graph)
        .collect::<Vec<_>>();
    let po_idx = HashMap::<G::Vertex, usize>::from_iter(
        postorder.iter().enumerate().map(|(a, b)| (b.clone(), a)),
    );
fn intersect<'a, V, E, G: 'a + Graph<'a,V,E> + BidirectionalGraph<'a,V,E> + VertexListGraph<'a,V,E>>(b1: G::Vertex,b2: G::Vertex,po_idx: &HashMap<G::Vertex,usize>, postorder: &Vec<G::Vertex>, ret: &HashMap<G::Vertex,G::Vertex> ) -> G::Vertex{
    ...
    while !fixpoint {
        ...
        for b in postorder.iter().rev().filter(|&&v| v != start) {
            ...
            let mut new_idom =
                *pred.iter().find(|&x| ret.contains_key(x)).unwrap();

            for p in pred.iter().filter(move |&&x| x != new_idom) {
                if ret.contains_key(&p) {
                    new_idom = intersect::<V, E, G>(
                        *p,
                        new_idom,
                        &po_idx,
                        &postorder,
