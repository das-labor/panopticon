...
pub fn binary_ptr(
    ...
    Box::new(
        move |st: &mut State<Avr>| {
            ...
            let addr_reg = Lvalue::Variable { name: Cow::Owned(reg_str), size: 16, subscript: None };
            ...
            st.mnemonic(
                    ...
                    sub (addr_reg), (addr_reg), [1]:16;
                        ...
                        stmts.append(&mut sem(addr_reg.clone(), reg.clone(), _cg)?);
    ...
    Box::new(
        move |st: &mut State<Avr>| {
            ...
            st.mnemonic(
                    ...
                    "{u}",
                    vec![rd.clone().into()],
                    &|_cg: &mut Mcu| -> Result<Vec<Statement>> { sem(rd.clone(), _cg) },
                )
                .unwrap();
            optional_skip(next.clone(), st);
            st.jump(next, Guard::always()).unwrap();
...
mod tests {
    ...
    use panopticon_core::{ControlFlowTarget, Function, Region, Rvalue};
    use panopticon_graph_algos::{BidirectionalGraphTrait, EdgeListGraphTrait, GraphTrait, IncidenceGraphTrait, VertexListGraphTrait};
    use std::borrow::Cow;
