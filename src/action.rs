...
impl Action {
    pub fn new_comment(
        panopticon: &mut Panopticon,
        address: u64,
        comment: String,
    ) -> Result<Action> {
        let maybe_func = panopticon.functions.iter().find(
            |f| {
                f.1.find_basic_block_at(address).is_some()
            }
        );
    ...
    pub fn new_setvalue(
        ...
        let value =
            value.map(
                |x| Kset::Set(x.into_iter().map(|x| (x, *len)).collect()),
            );

        let before = panopticon.control_flow_values.get(&func);
        let mut input =
            before.map(|x| x.input.clone()).unwrap_or(HashMap::new());
        ...
        let after = if input.is_empty() {
            ...
            let output = {
                let i = input.iter().map(
                    |(k, v)| {
                        ((k.name.clone(), k.subscript), v.clone())
                    }
                );
        ...
        let function = panopticon.functions.get(&func).unwrap();
        let addrs =
            diff_abstract_interpretations(after.as_ref(), before, &function);
    ...
    pub fn undo(&self, panopticon: &mut Panopticon) -> Result<()> {
        match self {
            &Action::Comment {
                ...
                panopticon.control_flow_comments.insert(
                    address,
                    before.clone(),
                );
                panopticon.update_control_flow_nodes(
                    function,
                    Some(&vec![address]),
                )
            }
            ...
            &Action::SetValue {
                ref function,
                ref before,
                ref modified_basic_blocks,
                ..
            } => {
                if let &Some(ref before) = before {
                    panopticon.control_flow_values.insert(
                        function.clone(),
                        before.clone(),
                    );
                } else {
...
fn diff_abstract_interpretations(
    a: Option<&AbstractInterpretation>,
    b: Option<&AbstractInterpretation>,
    func: &Function,
) -> Vec<u64> {
    ...
    let blocks = func.postorder()
        .into_iter()
        .filter_map(
            |vx| if let Some(&ControlFlowTarget::Resolved(ref bb)) =
    ...
    let mut ret = vec![];
    let nams = HashSet::<VarName>::from_iter(
        a.symmetric_difference(&b).map(|&(n, _)| n.clone()),
    );
