...
) -> Vec<(usize, f32, f32, bool, Vec<CBasicBlockLine>)> {
    nodes
        .into_iter()
...
fn transform_edges(edges: Vec<EdgePosition>)
    -> (Vec<u32>,
        Vec<CString>,
        Vec<CString>,
        Vec<f32>,
        ...
        .map(
            |(id, kind, label, (head_x, head_y), (tail_x, tail_y), segs)| {
                ...
                let svg = if let Some(&(x, y, _, _)) = segs.clone().next() {
                    ...
                    for (x, y) in segs.clone().take(1).map(&f).chain(
                        segs.clone()
                            .map(&g)
                    ) {
                    ...
                    let color = if kind == "fallthru" ||
                                   kind == "fallthru-backedge" {
                        "red"
                    } else if kind == "branch" ||
                              kind == "branch-backedge" {
                        "green"
                    } else {
                        "black"
                    };

                    let arrow = if let Some(&(_, _, x, y)) =
                        segs.clone().rev().next() {
                        let width = 12.;
                        let height = 8.;
...
fn transform_and_send_function(
    uuid: &Uuid,
    only_entry: bool,
    do_nodes: bool,
    do_edges: bool,
) -> future::BoxFuture<(), Error> {
    let uuid = uuid.clone();
    ...
    PANOPTICON
        ...
        .and_then(
            move |(nodes, edges)| {
                let uuid = uuid;
                let uuid = CString::new(uuid.clone().to_string().as_bytes())
                    .unwrap();
...
impl Glue for Qt {
    fn get_function(
        ...
        Self::send_layout_task(&CString::new(uuid.to_string()).unwrap())
            .unwrap();

        let task =
            transform_and_send_function(&uuid, only_entry, do_nodes, do_edges)
                .then(
                    |x| {
                        let uuid = CString::new("".to_string().as_bytes())
                            .unwrap();
    ...
    fn open_program(path: &str) -> glue::Result<()> {
        PANOPTICON.lock().open_program(path.to_string()).map_err(
            |e| {
                format!("{}", e).into()
            }
        )
    ...
    fn save_session(path: &str) -> glue::Result<()> {
        PANOPTICON.lock().save_session(path.to_string()).map_err(
            |e| {
                format!("{}", e).into()
            }
        )
    ...
    fn comment_on(address: u64, comment: &str) -> glue::Result<()> {
        PANOPTICON.lock().comment_on(address, comment.to_string()).map_err(
            |e| {
                format!("{}", e).into()
            }
        )
    ...
    fn rename_function(uuid: &Uuid, name: &str) -> glue::Result<()> {
        PANOPTICON
            .lock()
            .rename_function(uuid.to_string(), name.to_string())
