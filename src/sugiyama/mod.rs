...
pub use self::linear::{LinearLayout, linear_layout_initial_order,
                       linear_layout_order, linear_layout_placement,
                       linear_layout_rank, linear_layout_start};
...
mod tests {
    ...
    use sugiyama::rank::{add_virtual_vertices, ensure_single_entry,
                         remove_cycles, remove_loops};
    ...
    fn xcoord_computation() {
        ...
        let port_offset =
            HashMap::from_iter(graph.edges().map(|e| (e, (0., 0.))));
            ...
            for v in r.iter() {
                ...
                for w in r.iter() {
                    if v != w {
                        ...
                        if !(v_x + v_w / 2.0 < w_x - w_w / 2.0 ||
                             w_x + w_w / 2.0 < v_x - v_w / 2.0) {
                            if !coll.contains(&(*v, *w)) &&
                               !coll.contains(&(*w, *v)) {
                                println!(
                                    "in rank {} overlap between {} and {}",
                                    idx,
                                    v_lb,
                                    w_lb
                                );
    ...
    fn xcoord_computation_paper() {
        ...
        order.push(vec![v[0]]);
        rank.extend(
            order.last().unwrap().iter().map(
                |&vx| {
                    (vx, order.len() as isize - 1)
                }
            )
        );
        ...
        let mut ports =
            HashMap::from_iter(graph.edges().map(|e| (e, (0f32, 0f32))));
