...
use panopticon_graph_algos::{AdjacencyList, BidirectionalGraphTrait,
                             ...
                             IncidenceGraphTrait, MutableGraphTrait,
...
use sugiyama::rank::{add_virtual_vertices, compute_ranking,
                     ensure_single_entry, normalize_rank, remove_cycles,
                     remove_loops, remove_parallel_edges};
...
pub enum LinearLayout {
    ...
    Ordering {
        ...
        virt_start: usize,
        revd_edge_labels: HashSet<usize>,
        bipartite: HashMap<(usize, usize),
                           Vec<(AdjacencyListEdgeDescriptor,
...
#[cfg(test)]
pub fn linear_layout(vertices: &Vec<usize>, edges: &Vec<(usize, usize)>, dims: &HashMap<usize, (f32, f32)>, entry: Option<usize>, node_spacing: f32, rank_spacing: f32, port_spacing: f32, loop_spacing: f32, entry_spacing: f32, block_spacing: f32) -> Result<(HashMap<usize, (f32, f32)>, HashMap<usize, (Vec<(f32, f32, f32, f32)>, (f32, f32), (f32, f32))>), Error>{
...
pub fn linear_layout_start(
    vertices: &Vec<usize>,
    edges: &Vec<(usize, usize)>,
    entry: Option<usize>,
) -> Result<LinearLayout, Error> {
        ...
        let to_extend = graph
            ...
            .filter(
                |&e| {
                    ...
                    revd_edge_labels.contains(&elb) &&
                    (slb < virt_start || tlb < virt_start)
                }
            )
            .collect::<Vec<_>>();
        for e in to_extend {
            ...
            match (s_lb < virt_start, t_lb < virt_start) {
                ...
                (false, false) => {
                    return Err("Internal error while edge inverting".into())
                }
            }
        }
...
pub fn linear_layout_initial_order(layout: LinearLayout)
    -> Result<LinearLayout, Error> {
    if let LinearLayout::Ranked {
               graph,
               rev,
               head,
               virt_start,
...
pub fn linear_layout_order(mut layout: LinearLayout)
    -> Result<LinearLayout, Error> {
    match layout {
...
#[cfg(test)]
pub fn linear_layout_structural(
    vertices: &Vec<usize>,
    edges: &Vec<(usize, usize)>,
    entry: Option<usize>,
) -> Result<LinearLayout, Error> {
    ...
    loop {
        let cont =
            if let LinearLayout::Ordering { iterations_left, .. } = ordered {
                iterations_left > 0
            } else {
...
pub fn linear_layout_placement(vertices: &Vec<usize>, edges: &Vec<(usize, usize)>, layout: &LinearLayout, dims: &HashMap<usize, (f32, f32)>, node_spacing: f32, rank_spacing: f32, port_spacing: f32, loop_spacing: f32, entry_spacing: f32, block_spacing: f32) -> Result<(HashMap<usize, (f32, f32)>, HashMap<usize, (Vec<(f32, f32, f32, f32)>, (f32, f32), (f32, f32))>), Error>{
    if let &LinearLayout::Ordering {
        ...
        let mut x_off =
            HashMap::<AdjacencyListEdgeDescriptor, (f32, f32)>::from_iter(
                graph.edges().map(|e| (e, (0., 0.))),
            );
        ...
        for n in vertices.iter() {
            ...
            let mut down = graph
                ...
                .collect::<Vec<_>>();
            let sort_by_order = |a: &(AdjacencyListEdgeDescriptor,
                                      AdjacencyListVertexDescriptor),
                                 b: &(AdjacencyListEdgeDescriptor,
                                      AdjacencyListVertexDescriptor)|
             -> Ordering {
                ...
                if a.1 == vx {
                    ...
                    maybe_ord_b.partial_cmp(&maybe_ord_a).unwrap_or(
                        Ordering::Equal,
                    )
                }
            };
            ...
            if up.len() > 1 {
                let mut off = -1.0 * ((up.len() - 1) as f32) *
                              (port_spacing as f32) /
                              2.0;
                for w in up.iter() {
            ...
            if down.len() > 1 {
                let mut off = -1.0 * ((down.len() - 1) as f32) *
                              (port_spacing as f32) /
                              2.0;
                for w in down.iter() {
        ...
        for n in vertices.iter() {
            ...
            ret_v.insert(
                *n,
                (x_pos[&vx], rank_start + (rank_end - rank_start) / 2.0),
            );
        }

        // build end point list (edge label -> (start,end))
        let mut end_points = HashMap::<usize,
                                       (AdjacencyListVertexDescriptor,
                                        AdjacencyListVertexDescriptor)>::new();
        for vx in graph.vertices() {
            if *graph.vertex_label(vx).unwrap() < virt_start {
                for out in graph.out_edges(vx) {
                    end_points
                        .entry(*graph.edge_label(out).unwrap())
                        .or_insert((vx, vx))
                        .0 = vx;
                }
                for _in in graph.in_edges(vx) {
                    end_points
                        .entry(*graph.edge_label(_in).unwrap())
                        .or_insert((vx, vx))
                        .1 = vx;
                }
            }
        ...
        for lb in revd_edge_labels.iter() {
            if let Some(&(start, end)) = end_points.get(lb) {
                ...
                let post_start =
                    graph.target(
                        graph
                            .out_edges(start)
                            .find(
                                |&f| *graph.edge_label(f).unwrap() == *lb,
                            )
                            .unwrap()
                    );
                let pre_end =
                ...
                let ps_pos =
                    order[start_r].iter().position(|&x| start == x).unwrap();
                let s_pos = order[start_r]
                    .iter()
                    .position(|&x| post_start == x)
                    .unwrap();
                let s = order[start_r]
                    .iter()
                    ...
                    .fold(
                        0.0,
                        |acc, x| if acc < dims.get(x).unwrap_or(&(0., 0.)).1 {
                ...
                let pe_pos =
                    order[end_r].iter().position(|&x| end == x).unwrap();
                let e_pos =
                    order[end_r].iter().position(|&x| pre_end == x).unwrap();
                let e = order[end_r]
                    .iter()
            ...
            match maybe_e {
                ...
                Some(mut e) => {
                    ...
                    loop {
                        ...
                        let lb = graph.edge_label(e).unwrap();
                        let sx = x_pos[&s] +
                                 x_off.get(&e).map(|x| x.0).unwrap_or(0.0); // source center x
                        let tx = x_pos[&t] +
                                 x_off.get(&e).map(|x| x.1).unwrap_or(0.0); // target center x
                        ...
                        let ses = srm +
                                  dims.get(&s).map(|x| x.1).unwrap_or(0.) / 2. +
                                  block_spacing; // edge start x
                        let see = srm +
                                  dims.get(&s).map(|x| x.1).unwrap_or(0.) / 2. +
                                  block_spacing +
                                  entry_spacing; // edge entry part end x
                        let trs = rank_offsets[tr];
                        let tre = rank_offsets[tr + 1] - rank_sep;
                        let trm = trs + (tre - trs) / 2.;
                        let tee = trm -
                                  dims.get(&t).map(|x| x.1).unwrap_or(0.) / 2. -
                                  block_spacing -
                                  entry_spacing;
                        let tes = trm -
                                  dims.get(&t).map(|x| x.1).unwrap_or(0.) / 2. -
                    ...
                    ret_e.insert(
                        idx,
                        (ret,
                         start_arrow_off.unwrap(),
                         end_arrow_off.unwrap()),
                    );
                }
            }
        }
...
pub fn mark_type1_conflicts
    ...
    let mut ret = Vec::<(AdjacencyListVertexDescriptor,
                         AdjacencyListVertexDescriptor)>::new();
        ...
        for l1 in 0..order[next].len() {
            ...
            let maybe_upn = edges.find(
                |&e| {
                    ...
                    rank[&w] == i as isize &&
                    *graph.vertex_label(w).unwrap() >= virt_start &&
...
pub fn vertical_alignment(order: &Vec<Vec<AdjacencyListVertexDescriptor>>, type1: &Vec<(AdjacencyListVertexDescriptor, AdjacencyListVertexDescriptor)>, graph: &AdjacencyList<usize, usize>, up_to_down: bool, left_to_right: bool) -> (HashMap<AdjacencyListVertexDescriptor, AdjacencyListVertexDescriptor>, HashMap<AdjacencyListVertexDescriptor, AdjacencyListVertexDescriptor>){
    let mut root = HashMap::<AdjacencyListVertexDescriptor,
                             AdjacencyListVertexDescriptor>::new();
    let mut align = HashMap::<AdjacencyListVertexDescriptor,
                              AdjacencyListVertexDescriptor>::new();
    ...
    while i != to {
        ...
        while k != lev_to {
            assert!(
                i - delta >= 0 && i - delta < order.len() as isize &&
                i >= 0 && k >= 0
            );
            ...
            let upn = if up_to_down {
                order[prev]
                    .iter()
                    .filter(
                        |&&w| graph.out_edges(w).any(|e| graph.target(e) == v),
                    )
                    .cloned()
                    .collect::<Vec<_>>()
            } else {
                order[prev]
                    .iter()
                    .filter(
                        |&&w| graph.in_edges(w).any(|e| graph.source(e) == v),
                    )
                    .cloned()
                    .collect::<Vec<_>>()
            };
            ...
            if upn.len() > 0 {
                let medians = if upn.len() % 2 == 0 {
                    vec![
                        ((upn.len() - 1) as f32 / 2.).floor(),
                        ((upn.len() - 1) as f32 / 2.).ceil(),
                    ]
                } else {
...
fn calculate_threshold(
    ...
    old_threshold: f32,
    _: bool,
    queue: &mut Vec<(AdjacencyListVertexDescriptor,
                     AdjacencyListEdgeDescriptor)>,
    ...
    align: &HashMap<AdjacencyListVertexDescriptor,
                    AdjacencyListVertexDescriptor>,
    ...
    if v == w {
        ...
        if let Some(e) = graph.in_edges(w).next() {
            ...
            if placed.contains(&r) {
                threshold = x[&r] + inner_shift[&p] +
                            port_offset.get(&e).map(|x| x.0).unwrap_or(0.) -
                            inner_shift[&q] -
...
fn place_block(
    ...
    order: &Vec<Vec<AdjacencyListVertexDescriptor>>,
    rank: &HashMap<AdjacencyListVertexDescriptor, isize>,
    sink: &mut HashMap<AdjacencyListVertexDescriptor,
                       AdjacencyListVertexDescriptor>,
    ...
    if !x.contains_key(&v) {
        ...
        loop {
            assert!(rank[&w] >= 0);
            let word: &Vec<AdjacencyListVertexDescriptor> = &order[rank[&w] as
                                                             usize];
                ...
                if sink[&v] != sink[&u] {
                    ...
                    if !left_to_right {
                        shift.insert(
                            ...
                            partial_min(
                                prev,
                                x[&v] + inner_shift[&w] - x[&u] -
                                inner_shift[&n] -
                                dims.get(&n).unwrap_or(&(0., 0.)).0 -
                                spacing,
                            ),
                        );
                    } else {
...
fn horizontal_compaction(
    ...
    let mut sink = HashMap::<AdjacencyListVertexDescriptor,
                             AdjacencyListVertexDescriptor>::new();
    ...
    while vi != vto {
        ...
        if v == sink[&root[&v]] {
            ...
            if (!left_to_right && old_shift < f32::MAX) ||
               (left_to_right && old_shift > f32::MIN) {
...
pub fn inner_shift(root: &HashMap<AdjacencyListVertexDescriptor, AdjacencyListVertexDescriptor>, dims: &HashMap<AdjacencyListVertexDescriptor, (f32, f32)>, port_offset: &HashMap<AdjacencyListEdgeDescriptor, (f32, f32)>, graph: &AdjacencyList<usize, usize>) -> (HashMap<AdjacencyListVertexDescriptor, f32>, HashMap<AdjacencyListVertexDescriptor, f32>){
    let mut inner_shift =
        HashMap::from_iter(graph.vertices().map(|vx| (vx, 0.)));
    let mut left_right =
        HashMap::<AdjacencyListVertexDescriptor, (f32, f32)>::new();
    ...
    for e in graph.edges() {
        ...
        if r == root[&q] {
            let (mut left, mut right) =
                left_right.entry(r).or_insert((0., 0.)).clone();
            let s = inner_shift[&p] +
                    port_offset.get(&e).map(|x| x.0).unwrap_or(0.) -
                    port_offset.get(&e).map(|x| x.1).unwrap_or(0.);
    ...
    let block_size = HashMap::from_iter(
        left_right.into_iter().map(|(k, (l, r))| (k, (r - l))),
    );
...
pub fn compute_x_coordinates(
    ...
    let layouts = (0..4)
        .map(
            |k| {
                ...
                let (root, align) = vertical_alignment(
                    &order,
                    &type1,
                    &graph,
                    up_to_down,
                ...
                let (inner_shift, block_size) =
                    inner_shift(&root, &dims, &port_offset, &graph);
        ...
        .map(
            |(_, &(_, _, ref pos, ref inner_shift, _))| {
                ...
                let max = pos.iter()
                    .max_by(
                        |&(vx, x), &(wx, y)| {
                            (dims.get(vx).unwrap_or(&(0., 0.)).0 + x +
                             inner_shift[vx])
                                    .partial_cmp(
                                        &(dims.get(wx).unwrap_or(&(0., 0.)).0 +
                                          y +
                                          inner_shift[wx])
                                    )
                                    .unwrap_or(Ordering::Equal)
    ...
    let (smallest_layout, &((_, _), (_, _))) = min_max
        ...
        .enumerate()
        .min_by(
            |&(_, &((_, min1), (_, max1))), &(_, &((_, min2), (_, max2)))| {
                (max1 - min1).partial_cmp(&(max2 - min2)).unwrap_or(
                    Ordering::Equal,
                )
            }
        )
            ...
            .map(
                |vx| {
                    let mut candidate_pos = layouts
                        ...
                        .zip(shift.iter())
                        .map(
                            |(&(_, _, ref position, ref inner_shift, _),
                              shift)| position[&vx] + inner_shift[&vx] + shift
                        )
                        .collect::<Vec<_>>();

                    candidate_pos.sort_by(
                        |a, b| {
                            a.partial_cmp(&b).unwrap_or(Ordering::Equal)
                        }
                    );
                    (vx,
                     (candidate_pos[1] as f32 + candidate_pos[2] as f32) / 2.)
                }
            )
    );
