...
pub struct ControlFlowLayout {
    ...
    pub node_data:
        HashMap<AdjacencyListVertexDescriptor, (bool, Vec<BasicBlockLine>)>,
    pub edges: HashMap<AdjacencyListEdgeDescriptor,
...
impl ControlFlowLayout {
    pub fn new_async(
        ...
        let data = HashMap::from_iter(
            func.cflow_graph
                ...
                .filter_map(
                    |vx| {
                        func.cflow_graph.vertex_label(vx).map(|lb| (vx, lb))
                    }
                )
                .filter_map(
                    |(vx, lb)| {
                        let maybe_lines = Self::get_node_data(
                            lb,
                            comments,
                            values,
                            functions,
        ...
        let labels =
            HashMap::from_iter(
                func.cflow_graph
                    .edges()
                    .filter_map(
                        |e| Self::get_edge_data(e, func).ok().map(|x| (e, x)),
                    )
            );
        ...
        future::lazy(
            move || -> Result<_> {
                ...
                .and_then(
                    move |layout| {
                        future::loop_fn(
                            layout, |layout| if let &sugiyama::LinearLayout::Ordering { iterations_left: 0, .. } = &layout {
                ...
                .and_then(
                    move |layout| {
                        ...
                        future::lazy(
                            move || {
                                let mut placement = sugiyama::linear_layout_placement(&vertices.iter().map(|&vx| vx).collect::<Vec<_>>(), &edges, &layout, &dims, cmnt_width as f32 + 20., 20., 50., 30., 30., 8.)?;
                                    ...
                                    .fold(
                                        (min_x, min_y), |(min_x, min_y), (_, &(ref trail, _, _))| {
                                            let (x, y) = trail
                                                .iter()
                                                .fold(
                                                    (min_x, min_y), |(min_x, min_y), &(mut from_x, mut from_y, mut to_x, mut to_y)| {
                                                        from_x -= 10.;
                                                        from_y -= 10.;
                                                        to_x -= 10.;
                                                        to_y -= 10.;
    ...
    pub fn get_all_nodes
        (&self)
         -> Vec<(usize, f32, f32, bool, Vec<BasicBlockLine>)> {
        self.node_data
            .iter()
            .map(
                |(vx, data)| {
        ...
        for (&vx, &mut (_, ref mut lines)) in self.node_data.iter_mut() {
            let hit = if let Some(ref addrs) = addresses {
                lines
                    ...
                    .any(
                        |line| {
                            addrs.iter().find(|&&x| x == line.offset).is_some()
                        }
                    )
            } else {
                false
            ...
            if hit {
                ...
                let lb = cfg.vertex_label(vx)
                    .ok_or(
                        ::panopticon_core::Error("missing label in cfg".into()),
                    )?;
    ...
    fn flatten_cflow_graph(func: &Function)
        -> (HashSet<usize>,
            Vec<(usize, usize)>,
            HashMap<usize, AdjacencyListEdgeDescriptor>) {
        let mut edges = vec![];
        let cfg = &func.cflow_graph;
        ...
        let edges_rev =
            HashMap::<usize, AdjacencyListEdgeDescriptor>::from_iter(
                cfg.edges().enumerate(),
            );
        ...
        for vx in cfg.vertices() {
            ...
            match maybe_lb {
                Some(&ControlFlowTarget::Resolved(ref bb)) => {
                    ...
                    let height = line_count * line_height + 2 * margin +
                                 2 * padding;
                    let width = linew * char_width + 2 * margin + 2 * padding +
                                col_padding;
    ...
    fn get_node_data(
        ...
        match ct {
            ...
            &ControlFlowTarget::Unresolved(ref rv) => {
                Ok(vec![Self::get_value_line(rv, values)])
            }
    ...
    fn get_edge_data(
        edge_desc: AdjacencyListEdgeDescriptor,
        func: &Function,
    ) -> Result<(&'static str, String)> {
        let cfg = &func.cflow_graph;
        let label = cfg.edge_label(edge_desc)
            .map(
                |guard| if *guard != Guard::always() &&
                           *guard != Guard::never() {
                    format!("{}", guard)
                } else {
                    "".to_string()
