...
impl Panopticon {
    pub fn layout_function_async
        (
        &mut self,
        uuid: &Uuid,
    ) -> future::BoxFuture<(Vec<NodePosition>, Vec<EdgePosition>), Error> {
                ...
                150,
                    ...
                    .and_then(
                        move |cfl| {
                            ...
                            PANOPTICON.lock().control_flow_layouts.insert(
                                uuid,
                                cfl,
                            );
    ...
    fn get_function<'a>(
        &'a mut self,
        uuid: &Uuid,
    ) -> Result<&'a mut ControlFlowLayout> {
        if !self.control_flow_layouts.contains_key(&uuid) {
            let func = self.functions.get(&uuid).unwrap();
            let cmnts = &self.control_flow_comments;
            let values = self.control_flow_values.get(&uuid);
            let funcs = &self.functions;
            let cfl = ControlFlowLayout::new(
                func,
                cmnts,
                values,
                funcs,
    ...
    pub fn open_program(&mut self, path: String) -> Result<()> {
        ...
        if let Ok(proj) = Project::open(&Path::new(&path)) {
            if !proj.code.is_empty() {
                {
                    ...
                    for f in cg.vertices() {
                        if let Some(&CallTarget::Concrete(ref func)) =
                            cg.vertex_label(f) {
        ...
        } else if let Ok((mut proj, machine)) =
            ...
            if let Some(prog) = maybe_prog {
                ...
                let dir = format!(
                    "{}",
                    TempDir::new_in(dir, "panop-backing")
                        .unwrap()
                        .path()
                        .display()
                );
                ...
                Err(format!("{} is neither a saved session nor a recognized executable type", path).into())
            }
        } else {
            Err(format!("{} is neither a saved session nor a recognized executable type", path).into())
        }
    ...
    pub fn set_value_for(
        ...
        let toks: Vec<String> =
            variable.split('_').map(str::to_string).collect();
        if toks.len() == 2 {
            if let Ok(subscript) = usize::from_str(&toks[1]) {
                    ...
                    let vals = value
                        ...
                        .filter_map(
                            |x| {
                                ...
                                if x.starts_with("0x") {
                                    let s: &[_] = &['0', 'x'];
                                    u64::from_str_radix(x.trim_matches(s), 16)
                                        .ok()
                                } else if x.starts_with("0b") {
                                    let s: &[_] = &['0', 'b'];
                                    u64::from_str_radix(x.trim_matches(s), 2)
                                        .ok()
                                } else {
                                    u64::from_str(x).ok()
                                }
                ...
                let act = Action::new_setvalue(
                    self,
                    Uuid::parse_str(&func).unwrap(),
                    var,
                    val,
                )?;
                self.push_action(act)?;
    ...
    pub fn undo(&mut self) -> Result<()> {
        ...
        if top == 0 || self.undo_stack.get(top - 1).is_none() {
            return Err(
                format!("call to undo() when canUndo() is false").into(),
            );
        }
    ...
    pub fn redo(&mut self) -> Result<()> {
        ...
        if self.undo_stack.get(top).is_none() {
            return Err(
                format!("call to redo() when canRedo() is false").into(),
            );
        }
    ...
    pub fn update_control_flow_nodes(
        &mut self,
        uuid: &Uuid,
        addrs: Option<&[u64]>,
    ) -> Result<()> {
        ...
        let ids = if let Some(ref mut cfl) =
            self.control_flow_layouts.get_mut(uuid) {
            let func = self.functions.get(&uuid).unwrap();
            let cmnts = &self.control_flow_comments;
            let values = self.control_flow_values.get(&uuid);
            let funcs = &self.functions;
        ...
        let bbls: Vec<_> = self.get_function_nodes(uuid.clone().to_string())
            ...
            .into_iter()
            .filter_map(
                |bbl| if bbl.4.is_empty() || ids.is_empty() ||
                         ids.iter().any(|&x| x == bbl.0 as i32) {
                    Some(bbl)
                } else {
                    None
                }
    ...
    pub fn new_function(&mut self, func: Function) -> Result<()> {
        ...
        let pairs = {
            let maybe_entry =
                {
                    ...
                    if let Some(&ControlFlowTarget::Resolved(ref bb)) =
                        func.entry_point.and_then(|x| cfg.vertex_label(x)) {
                        Some(bb.area.start)
                    } else {
                        None
                    }
            ...
            if let Some(entry) = maybe_entry {
                // my addr
                let pairs_owned = self.unresolved_calls
                    .remove(&Some(entry))
                    .unwrap_or(vec![])
                    .into_iter();
                let pairs_ref = self.unresolved_calls
                    .get_vec(&None)
                    .cloned()
                    .unwrap_or(vec![])
                    .into_iter();
