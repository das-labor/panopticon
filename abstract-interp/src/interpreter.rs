...
use panopticon_core::{ControlFlowGraph, ControlFlowRef, ControlFlowTarget,
                      Function, Guard, Lvalue, Operation, Result, Rvalue,
                      Statement, lift};
...
pub fn approximate<A: Avalue>(
    func: &Function,
    fixed: &HashMap<(Cow<'static, str>, usize), A>,
) -> Result<HashMap<Lvalue, A>> {
        ...
        let head = if let Some(h) = h.first() {
            match &**h {
                ...
                &HierarchicalOrdering::Component(ref vec) => {
                    return stabilize(vec, graph, constr, sizes, ret, fixed)
                }
            }
        } else {
            return Ok(());
    ...
    for vx in func.cflow_graph.vertices() {
        for e in func.cflow_graph.in_edges(vx) {
            if let Some(&Guard::Predicate { .. }) =
                func.cflow_graph.edge_label(e) {
                match edge_ops.get(&e).cloned() {
                    Some(Operation::Equal(left @ Rvalue::Constant {
                                              ..
                                          },
                                          right @ Rvalue::Variable {
                                              ..
                                          })) => {
                        constr.insert(
                            Lvalue::from_rvalue(right).unwrap(),
                            A::abstract_constraint(
                                &Constraint::Equal(left.clone()),
                            ),
                        );
                    }
                    Some(Operation::Equal(left @ Rvalue::Variable {
                                              ..
                                          },
                                          right @ Rvalue::Constant {
                                              ..
                                          })) => {
                        constr.insert(
                            Lvalue::from_rvalue(left).unwrap(),
                            A::abstract_constraint(
                                &Constraint::Equal(right.clone()),
                            ),
                        );
                    }
                    ...
                    Some(Operation::LessSigned(left @ Rvalue::Constant {
                                                   ..
                                               },
                                               right @ Rvalue::Variable {
                                                   ..
                        ...
                        constr.insert(
                            Lvalue::from_rvalue(right).unwrap(),
                            A::abstract_constraint(
                                &Constraint::LessSigned(left.clone()),
                            ),
                        );
                    }
                    Some(Operation::LessSigned(left @ Rvalue::Variable {
                                                   ..
                                               },
                                               right @ Rvalue::Constant {
                                                   ..
                        ...
                        constr.insert(
                            Lvalue::from_rvalue(left).unwrap(),
                            A::abstract_constraint(
                                &Constraint::LessSigned(right.clone()),
                            ),
                        );
                    }
    ...
    Ok(
        HashMap::from_iter(
            ret.iter()
                .filter_map(
                    |(&(ref name, ref subscript), val)| if let Some(sz) =
                        sizes.get(name) {
                        Some(
                            (Lvalue::Variable {
                                 name: name.clone(),
...
pub fn results<A: Avalue>(
    func: &Function,
    vals: &HashMap<Lvalue, A>,
) -> HashMap<(Cow<'static, str>, usize), A> {
...
mod tests {
    use super::*;
    use panopticon_core::{BasicBlock, Bound, ControlFlowGraph, ControlFlowTarget, Function,
         Guard, Lvalue, Mnemonic, Operation, Rvalue, Statement};
    ...
    impl Avalue for Sign {
        ...
        fn abstract_constraint(c: &Constraint) -> Self {
            match c {
                &Constraint::Equal(Rvalue::Constant { value: 0, .. }) => {
                    Sign::Zero
                }
                &Constraint::LessUnsigned(Rvalue::Constant {
                                             value: 1, ..
                                         }) => Sign::Zero,
                &Constraint::LessOrEqualUnsigned(Rvalue::Constant {
                                                    value: 0, ..
                                                }) => Sign::Zero,
                &Constraint::LessSigned(Rvalue::Constant {
                                           value: 0, ..
                                       }) => Sign::Negative,
                &Constraint::LessOrEqualSigned(Rvalue::Constant {
                                                  value: v,
                                                  size: s,
                                              }) if s <= 64 && v & (1 << (s - 1)) != 0 => {
                    Sign::Negative
        ...
        fn execute(_: &ProgramPoint, op: &Operation<Self>) -> Self {
            match op {
                ...
                &Operation::Subtract(Sign::Positive, Sign::Positive) => {
                    Sign::Join
                }
                &Operation::Subtract(Sign::Positive, Sign::Zero) => {
                    Sign::Positive
                }
                &Operation::Subtract(Sign::Zero, Sign::Positive) => {
                    Sign::Negative
                }
                &Operation::Subtract(Sign::Negative, Sign::Negative) => {
                    Sign::Join
                }
                &Operation::Subtract(Sign::Negative, Sign::Zero) => {
                    Sign::Negative
                }
                &Operation::Subtract(Sign::Zero, Sign::Negative) => {
                    Sign::Positive
                }
                &Operation::Subtract(Sign::Positive, Sign::Negative) => {
                    Sign::Positive
                }
                &Operation::Subtract(Sign::Negative, Sign::Positive) => {
                    Sign::Negative
                }
                ...
                &Operation::Multiply(Sign::Positive, Sign::Positive) => {
                    Sign::Positive
                }
                &Operation::Multiply(Sign::Negative, Sign::Negative) => {
                    Sign::Positive
                }
                &Operation::Multiply(Sign::Positive, Sign::Negative) => {
                    Sign::Negative
                }
                &Operation::Multiply(Sign::Negative, Sign::Positive) => {
                    Sign::Negative
                }
                ...
                &Operation::DivideSigned(Sign::Positive, Sign::Positive) => {
                    Sign::Positive
                }
                &Operation::DivideSigned(Sign::Negative, Sign::Negative) => {
                    Sign::Positive
                }
                &Operation::DivideSigned(Sign::Positive, Sign::Negative) => {
                    Sign::Negative
                }
                &Operation::DivideSigned(Sign::Negative, Sign::Positive) => {
                    Sign::Negative
                }
                ...
                &Operation::DivideUnsigned(Sign::Positive, Sign::Positive) => {
                    Sign::Positive
                }
                &Operation::DivideUnsigned(Sign::Negative, Sign::Negative) => {
                    Sign::Positive
                }
                &Operation::DivideUnsigned(Sign::Positive, Sign::Negative) => {
                    Sign::Negative
                }
                &Operation::DivideUnsigned(Sign::Negative, Sign::Positive) => {
                    Sign::Negative
                }
                ...
                &Operation::Modulo(Sign::Positive, Sign::Positive) => {
                    Sign::Positive
                }
                &Operation::Modulo(Sign::Negative, Sign::Negative) => {
                    Sign::Positive
                }
                &Operation::Modulo(Sign::Positive, Sign::Negative) => {
                    Sign::Negative
                }
                &Operation::Modulo(Sign::Negative, Sign::Positive) => {
                    Sign::Negative
                }
            ...
            vec![
                Mnemonic::new(
                    ...
                    vec![
                        Statement {
                            op: Operation::Move(
                                Rvalue::new_u64(0xffffffffffffff00),
                            ),
