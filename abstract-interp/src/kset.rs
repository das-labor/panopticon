...
impl Avalue for Kset {
        ...
        match *op {
            ...
            Operation::ShiftRightSigned(ref a, ref b) => {
                permute(
                    a,
                    b,
                    &|a, b| execute(Operation::ShiftRightSigned(a, b)),
                )
            }
            Operation::ShiftRightUnsigned(ref a, ref b) => {
                permute(
                    a,
                    b,
                    &|a, b| execute(Operation::ShiftRightUnsigned(a, b)),
                )
            }
            ...
            Operation::LessOrEqualSigned(ref a, ref b) => {
                permute(
                    a,
                    b,
                    &|a, b| execute(Operation::LessOrEqualSigned(a, b)),
                )
            }
            Operation::LessOrEqualUnsigned(ref a, ref b) => {
                permute(
                    a,
                    b,
                    &|a, b| execute(Operation::LessOrEqualUnsigned(a, b)),
                )
            }
    ...
    fn combine(&self, a: &Self) -> Self {
        match (self, a) {
            ...
            (&Kset::Meet, b) => b.clone(),
            (&Kset::Set(ref a), &Kset::Set(ref b)) => {
                let mut ret = HashSet::<&(u64, usize)>::from_iter(
                    a.iter().chain(b.iter()),
                )
                        .iter()
                        .cloned()
...
mod tests {
    use super::*;
    use panopticon_core::{BasicBlock, ControlFlowGraph, ControlFlowTarget, Function, Guard,
         Lvalue, Mnemonic, Operation, Rvalue, Statement};
